 ...
 *
 * The presenter receives a full legacy or Watch-native projection only at the application boundary
 * and exposes only [StapleWatchUiState] to the renderer. Opaque store keys, exact economic
 * decisions, arithmetic, freshness authority, notification authorization and provider metadata
...
class StapleWatchSurfacePresenter(
    ...
    fun render(projection: StapleWatchBasketUiProjection) {
        renderer.render(projection.state)
    }
}

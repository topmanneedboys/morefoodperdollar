...
class CoreBoundaryTest {
    ...
    fun comparisonActivityUsesExactReplaceableCompareHereBoundary() {
        ...
        assertTrue(source.contains("privateMemoryStateWhenStatusRendered"))
        assertTrue(source.contains("val loaded = privateMemoryStore.load()"))
        assertTrue(source.contains("if (!loaded.accepted || loaded.state != expected)"))
        assertTrue(source.contains("privateMemoryStateWhenStatusRendered = null"))
        assertTrue(source.contains("privateMemoryStateWhenStatusRendered = requireNotNull(result.state)"))
        assertTrue(source.contains("hidePrivateMemoryStatus()"))

...
The promoted milestone is now `2ec22231515b37639aa596fb07edf71704cc1fb9` (`Explain unavailable good-price history`). Candidate workflow **33854375766** and milestone provenance workflow **33854895645** passed for the exact same SHA. Home now has a dedicated single-product “Is this a good price?” route. It accepts one bounded manual entry, delegates parsing, selected-price semantics and exact unit math to the existing Compare Here bridge, and presents a private-history answer only: no history, last-price comparison, or personal range. Exact currency, package quantity, current/member basis and exact promotion facts are required; unknown or conflicting facts do not become a recommendation. A successful user check emits a source-labelled `CONFIRMED_GOOD_PRICE_CHECK` entry into the same capped, atomic, integrity-checked device memory, while preserving source provenance and never feeding planner/ranking/current-offer authority. If private history cannot be read, the screen explicitly says history is unavailable and offers local recovery instead of silently presenting an empty-history answer. Home’s new action is owner-gated and the dedicated screen remains offline, private, and explicit that it is not live store pricing. The earlier confirmed Compare Here memory, review-first photo import, fictional sample disclosure, 30,000-record identity rail and atomic catalog release remain unchanged.

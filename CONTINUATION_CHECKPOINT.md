...
The promoted milestone is now `59d565e79c7fa6e3f5a0b28acb5fc7920bb1c5c0` (`Record deterministic offline catalog coverage`), verified by candidate workflow **33814661112**, offline-catalog workflow **33814661056**, and milestone provenance workflow **33815157624**. The weekly offline catalog coordinator now emits an atomic, deterministic `coverage-report.json` that separates selected identity-catalog coverage from current-offer coverage (`0` / `NOT_INCLUDED`) for each requested metro, while signed manifests and promotion pointers remain authoritative. A candidate’s current-offer metadata is also checked explicitly before staging. This is diagnostic/data-foundation work only: it does not add prices, offers, availability, planner/ranking authority, Android networking, or any new consumer claim. Home’s preceding offline catalog dialog still lets the shopper explicitly select a bounded display name, replaces only the first whole unresolved query token, and returns the edited list for review before Plan My Shop. Clean-source verification passed 400 shared-core tests and 1,337 Android app tests with zero failures, all 58 Android tasks, 61 Python tests, 30 browser tests, Firefox lint with zero findings, APK privacy inspection with no INTERNET/ACCESS_NETWORK_STATE, one-signer verification and exact-SHA release-bundle provenance.

...
The promoted milestone is now `5789448b3836d5d6fe8757209cf47fe0fe067181` (`Trigger full verification for merchant feed qualification`), verified by candidate workflow **33817795995** and milestone provenance workflow **33818227384**. Home’s offline catalog handoff states that selecting a bundled identity name only edits the Home list; because the planner is still a tiny fictional sample vocabulary, the inserted name may remain unresolved. The existing identity-only, no-price/no-availability disclosure remains intact, and catalog selections remain query-only until the separate exact-identity confirmation boundary is deliberately wired. The merchant-feed qualifier now reports identity coverage separately from structural current-offer and unit-value candidate counts; those latter measurements remain `STRUCTURAL_ONLY` with authority `NONE`, so they cannot become current-price, availability, or organic-ranking facts. The Android build workflow now includes these qualifier paths in its push/PR filters so future changes receive automatic full verification. This adds no prices, offers, availability, planner/ranking authority, Android networking, or any new identity claim. The prior weekly offline catalog coordinator still emits the atomic diagnostic `coverage-report.json`, with signed manifests and promotion pointers authoritative and current-offer coverage explicitly `0` / `NOT_INCLUDED`. Clean-source verification passed 400 shared-core tests and 1,337 Android app tests with zero failures, all 58 Android tasks, 62 Python tests, 30 browser tests, Firefox lint with zero findings, APK privacy inspection with no INTERNET/ACCESS_NETWORK_STATE, one-signer verification, exact-SHA release-bundle provenance, and exact-SHA merchant-qualifier workflows **33818313377** (candidate) and **33818315651** (milestone).

...
`2e2e526178043eb8e8d534df64ed1a380b6ff07e` (`Restore Home item details drafts`) is the promoted milestone, verified by candidate workflow **33750002662** and milestone provenance workflow **33750549290**. The Home item-details editor now preserves an in-progress package-count, brand and exact-product draft across Android Activity recreation through ephemeral `onSaveInstanceState` only. Restoration is restricted to the Home route and an item still present in the restored request; the draft never enters the persistent Home session or planner until the shopper explicitly saves it. This is presentation/lifecycle-only and does not alter the typed Home session, planner, ranking, pricing, evidence, persistence, provider, provider-economics or network authority. Clean-source verification passed 1,690 JVM tests (375 shared-core + 1,315 Android app), all 58 Android tasks, all 30 browser tests, Firefox lint with zero findings, APK privacy checks with no network permissions, one-signer verification and release-bundle provenance.

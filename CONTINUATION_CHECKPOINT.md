...
`ad18ea11a5d70b084801c423bcb3c33e167ee469` (`Announce projected shopping plans to accessibility services`) is the promoted milestone, verified by candidate workflow **33710068590** and milestone provenance workflow **33710642233**. The shared projected Home/Basket result container is now a polite accessibility live region, so TalkBack can hear when a newly planned recommendation replaces the empty result area. Existing card-level summaries still carry the exact projected badge, store, cost/coverage, travel, evidence and fictional-sample context. The change is presentation-only and leaves immutable plan state, typed actions, and all existing fail-closed controls unchanged. Home’s query wrapper remains gated when detached; Search’s changing ready/loading/results/error status, Basket’s local check-off progress, Saved lifecycle status and Home refinement feedback remain announced through their existing live regions. Basket still distinguishes a ready-but-unsubmitted list from an incomplete one with “Plan this list on Home.” No planner, ranking, pricing, evidence, persistence, clock, provider or network authority moved into the View. Clean-source verification passed 1,656 JVM tests (375 shared-core + 1,281 Android app), all 58 Android tasks, all 30 browser tests, Firefox lint with zero findings, APK privacy checks with no network permissions, one-signer verification and release-bundle provenance.

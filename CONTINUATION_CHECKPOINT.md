...
Latest promoted head: `c2bb5f64d69d617ce898cf6f281ecf78c67d7e21` (`Explain Watch My Staples entry point`), candidate workflow **33670610252**, milestone workflow **33671233369**. The current app slice adds a renderer-ready Saved Watch entry explanation: a valid launcher now presents fixed copy describing recurring saved items, a usual store, and checking whether a future switch is worth the trip. The copy is navigation context only and makes no live-price, alert, or notification claim. The preceding Watch setup selection summary remains intact: the selected staple count is shown against the existing two-item handoff minimum, alongside whether a usual store has been selected. Both summaries are derived by immutable presentation; physical Views only render supplied strings and emit typed navigation/selection actions. The preceding bounded local-only cross-session persistence around `PracticalShoppingHomeSession.State` remains intact: the last Home list, chicken choice, extra-stop preference and opaque request-details bytes are restored only for the exact established request; missing/oversized/corrupt values fail closed, and unsubmitted drafts do not persist stale details. Existing per-item package count, brand and exact-product controls remain separate from planner arithmetic. No Watch setup copy or Home details feed planner, ranking, pricing, evidence, clock, networking, or View authority. The provenance proof accepts both `candidate/*` and historical `work/valuepilot-*` candidates but excludes the milestone branch. Full candidate Android/browser/privacy/signature/release checks and milestone provenance passed.

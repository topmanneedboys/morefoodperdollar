...
The promoted milestone is now `52211c9a68e7c7537c872f5d62a48513de2d5122` (`Make offline Home catalog matches actionable`), verified by candidate workflow **33811804791** and milestone provenance workflow **33812349630**. Home’s offline catalog dialog now lets the shopper explicitly select a bounded display name, replaces only the first whole unresolved query token, and returns the edited list for review before the shopper presses Plan My Shop. Invalid, unsafe, or overlong edits fail closed. This is a reversible query edit only: it does not confirm product identity, submit planning, add prices, claim availability, or change planner/ranking authority. The underlying catalog remains identity-only, Canada-labelled Open Food Facts metadata for the supported Greater Toronto Area and Metro Vancouver snapshots, with deterministic deduplication, conflict omission, bounds, rights/timestamp/signature admission, and fail-closed regional coverage. Clean-source verification passed 400 shared-core tests and 1,337 Android app tests with zero failures, all 58 Android tasks, 61 Python tests, 30 browser tests, Firefox lint with zero findings, APK privacy inspection with no INTERNET/ACCESS_NETWORK_STATE, one-signer verification and exact-SHA release-bundle provenance.

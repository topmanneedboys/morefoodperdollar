...
The promoted milestone is now `578aacf95521d3517da4260b78c7da01666374fb` (`Identify unresolved item in offline match dialog`), verified by candidate build workflow **33825664697** and milestone provenance workflow **33826037482**. The offline-match dialog now identifies the exact unresolved Home word it will search/replace in its title, including loading, empty and selectable-result states; this gives users and screen readers clear context when multiple list words need clarification. The bundled Greater Toronto Area and Metro Vancouver identity snapshots contain 5,000 records each, 4,199 unique canonical identity names (`WITHIN_TARGET` for the 1,500–5,000 launch band), 4,960 grocery-hinted records, and 41 household-hinted records protected by the deterministic 10% household reserve. Current offers remain explicitly `0` / `NOT_INCLUDED`. Home’s offline catalog handoff states that selecting a bundled identity name only edits the Home list; because the planner is still a tiny fictional sample vocabulary, the inserted name may remain unresolved. An empty catalog result explicitly says that a miss in this snapshot does not mean the product is unavailable. The existing identity-only, no-price/no-availability disclosure remains intact, and catalog selections remain query-only until the separate exact-identity confirmation boundary is deliberately wired. The weekly selector’s default bounded maximum is now 5,000; it recognizes household category branches plus conservative, context-checked household name hints, keeps that bounded reserve, and persists selection measurements into the atomic `coverage-report.json`; false-positive guards are covered by deterministic tests. These are identity/category measurements only, not demand, retailer availability, stock, price, package quantity, freshness, or ranking authority. The merchant-feed qualifier still reports identity coverage separately from structural current-offer and unit-value candidate counts; those latter measurements remain `STRUCTURAL_ONLY` with authority `NONE`. The Android build workflow includes both qualifier paths in its push/PR filters. This adds no prices, offers, availability, planner/ranking authority, Android networking, or any new identity claim. Signed manifests and promotion pointers remain authoritative, and rejected regressions preserve the prior report and last-known-good pointers. Clean-source verification passed 400 shared-core tests and 1,337 Android app tests with zero failures, all 58 Android tasks, 64 Python tests, 30 browser tests, Firefox lint with zero findings, APK privacy inspection with no INTERNET/ACCESS_NETWORK_STATE, one-signer verification, exact-SHA release-bundle provenance, and exact-SHA offline-catalog workflows **33824608971** (candidate) and **33825032353** (milestone).

...
The next candidate slice is a recovery-path correction on top of `6743181`: when Home cannot read
private comparison history, it now keeps the typed `Review private price history` action visible so
the shopper can open Scan & compare and use its existing recovery/clear controls. Home still
suppresses every unreadable history-derived row and summary, and the action carries no price,
offer, store, availability, planner, ranking, network, provider-economics or persistence authority.
Focused tests pass. Full Windows verification still reports the repository's known line-ending-
sensitive source-boundary failures; lint/build, browser checks and the Python/catalog suite pass.

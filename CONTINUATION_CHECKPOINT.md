...
The promoted milestone is now `f1ea903b5400242b20c73b1ae683c9f226bf2bfa` (`Add user initiated camera capture to price comparison`). Candidate workflow **33859783985** and milestone provenance workflow **33860267357** passed for the exact same SHA. Scan & Compare now offers a user-triggered `Take a price photo` action beside photo import. CAMERA is requested only after the tap; a temporary JPEG is written under the app cache through a FileProvider URI with explicit temporary grants, then reused by the existing bounded on-device OCR and review flow. A shared in-flight gate disables both photo actions, stale/lifecycle callbacks are ignored, temporary capture files are deleted on all terminal paths and teardown, and unavailable/denied/error states preserve existing entries. The OCR status is a polite accessibility live region. This is a consumer-flow improvement only: no Android networking, live-price authority, stock/availability claim, planner/ranking authority, provider economics or persistent overlay foundation was added. The existing private comparison memory, Home unavailable-history disclosure, good-price route, fictional sample disclosure, 30,000-record identity rail and atomic catalog release remain unchanged.

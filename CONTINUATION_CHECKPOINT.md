...
The promoted milestone is now `27d1807a49aab4cb05c41bcbb21a8d7b975322a3` (`Add household coverage to offline catalog refresh`), verified by candidate build workflow **33820392525**, offline-catalog workflow **33820392502**, and milestone provenance workflow **33820796885**. The bundled GTA and Metro Vancouver identity snapshots contain 3,000 records each, 2,540 unique canonical identity names (`WITHIN_TARGET` for the 1,500–5,000 launch band), 2,983 grocery-hinted records, and 18 household-hinted records protected by the deterministic 10% household reserve; current offers remain explicitly `0` / `NOT_INCLUDED`. Home’s offline catalog handoff states that selecting a bundled identity name only edits the Home list; because the planner is still a tiny fictional sample vocabulary, the inserted name may remain unresolved. The existing identity-only, no-price/no-availability disclosure remains intact, and catalog selections remain query-only until the separate exact-identity confirmation boundary is deliberately wired. The weekly selector now recognizes household category branches, keeps that bounded reserve, and persists the selection measurements into the atomic `coverage-report.json`; these are identity/category measurements only, not demand, retailer availability, stock, price, package quantity, freshness, or ranking authority. The merchant-feed qualifier still reports identity coverage separately from structural current-offer and unit-value candidate counts; those latter measurements remain `STRUCTURAL_ONLY` with authority `NONE`. The Android build workflow includes both qualifier paths in its push/PR filters. This adds no prices, offers, availability, planner/ranking authority, Android networking, or any new identity claim. Signed manifests and promotion pointers remain authoritative, and rejected regressions preserve the prior report and last-known-good pointers. Clean-source verification passed 400 shared-core tests and 1,337 Android app tests with zero failures, all 58 Android tasks, 63 Python tests, 30 browser tests, Firefox lint with zero findings, APK privacy inspection with no INTERNET/ACCESS_NETWORK_STATE, one-signer verification, exact-SHA release-bundle provenance, and exact-SHA offline-catalog workflows **33820392502** (candidate) and **33820796799** (milestone).

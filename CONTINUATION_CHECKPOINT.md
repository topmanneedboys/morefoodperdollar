...
`9fa17302a8409f6c991c9761f0ac0fda3e08903f` (`Name Watch selection actions`) is the promoted milestone, verified by candidate workflow **33682834699** and milestone workflow **33683298432**. Watch product and usual-store selection rows now carry immutable, item-specific accessibility descriptions for their toggle actions, while the existing typed selection actions and readiness gates remain unchanged. The descriptions are renderer-ready copy; the View only binds them and emits the existing typed selection action. Saved removal actions remain item-specific, Watch policy editors remain field-specific, and Home item details remain explicitly preference-only without changing the fictional sample total. Clean-source verification passed 1,634 JVM tests (375 shared-core + 1,259 Android app), all 58 Android tasks, all 30 browser tests, APK privacy checks, one-signer verification and release-bundle provenance.

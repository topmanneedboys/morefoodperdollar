...
`bd35d8a2a042226013ce6b93e0c60767a2c1d0e6` (`Collapse inert Home extra-stop panel`) is the promoted milestone, verified by candidate workflow **33733994101** and milestone provenance workflow **33734530262**. Home now collapses the already-expanded extra-stop settings panel when its typed owner callback disappears, so a detached or reused renderer cannot leave an inert interactive-looking panel on screen. This is presentation/lifecycle-only: provider execution, ranking, evidence, persistence, clock, provider economics and network authority remain unchanged. Clean-source verification passed 1,681 JVM tests (375 shared-core + 1,306 Android app), all 58 Android tasks, all 30 browser tests, Firefox lint with zero findings, APK privacy checks with no network permissions, one-signer verification and release-bundle provenance.

## Previous verified engineering head (superseded)

`4bc6a0ab50e4a4a82f351c4ff5b60ac601f2d703` (`Bound Search received observation state`) was the promoted milestone, verified by candidate workflow **33732943799** and milestone provenance workflow **33733488198**. Search now reports only the bounded provider observations admitted for evaluation, never an oversized raw batch count; parsing, ranking, visible results, evidence and provider authority are unchanged. This is controller-state/bounds-only: provider execution, ranking, evidence, persistence, clock, provider economics and network authority remain unchanged. Clean-source verification passed 1,681 JVM tests (375 shared-core + 1,306 Android app), all 58 Android tasks, all 30 browser tests, Firefox lint with zero findings, APK privacy checks with no network permissions, one-signer verification and release-bundle provenance.

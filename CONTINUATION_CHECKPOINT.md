...
`92ffa4c92d903f5de47a8be5dbc34e54380cd599` (`Make saved shell copy state safe`) is the promoted milestone, verified by candidate workflow **33695634770** and milestone provenance workflow **33696077104**. Home and Basket now fail closed when a recognized request has no usable price coverage: the planner’s “not enough price coverage” result remains visible, the extra-stop rule is hidden because there is no primary store plan, and Basket labels the state “Price coverage needed” instead of presenting it as a usable basket plan. The Basket shell now uses state-safe copy—“Review your shopping list” and “Review the shopping list from Home”—and hides the generic check-off progress line until a primary store plan exists. The Saved shell now uses lifecycle-safe copy—“Manage saved choices” and a neutral management description—so loading, empty and error states do not imply confirmed choices are already present. Existing complete and incomplete-primary behavior is unchanged, including typed check-off eligibility and the local-only check-off notice. No planner, ranking, pricing, evidence, persistence, clock, provider or network authority moved into the View. Clean-source verification passed 1,642 JVM tests (375 shared-core + 1,267 Android app), all 58 Android tasks, all 30 browser tests, Firefox lint with zero findings, APK privacy checks with no network permissions, one-signer verification and release-bundle provenance.

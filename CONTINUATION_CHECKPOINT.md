...
`9304aa29bccf06e3df8adf48d5f8943af838eb68` (`Announce Basket check-off progress to assistive technology`) is the promoted milestone, verified by candidate workflow **33707613303** and milestone provenance workflow **33708102326**. Basket’s local check-off progress is now a polite accessibility live region, so TalkBack can hear the projected “n of m items collected” update after each typed foreground action or reset. The change is presentation-only and leaves immutable lifecycle/progress state, typed actions, and all existing fail-closed controls unchanged. Saved and Home status feedback remain announced through their live regions, and Basket still distinguishes a ready-but-unsubmitted list from an incomplete one with “Plan this list on Home.” The fictional/offline sample disclosure, accessibility summaries, Saved confirmation, item-preference gating, no-coverage correction, and state-safe Basket/Saved shell copy remain intact. No planner, ranking, pricing, evidence, persistence, clock, provider or network authority moved into the View. Clean-source verification passed 1,653 JVM tests (375 shared-core + 1,278 Android app), all 58 Android tasks, all 30 browser tests, Firefox lint with zero findings, APK privacy checks with no network permissions, one-signer verification and release-bundle provenance.

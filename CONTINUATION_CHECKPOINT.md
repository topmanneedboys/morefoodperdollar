...
Latest promoted head: `79c754b3468aa6e0927e83a6c9f98ddd3e3c0947` (`Persist Home item intent details`), candidate workflow **33664441555**, milestone workflow **33665119468**. The current app slice adds typed `PracticalShoppingHomeSession.State` around the existing immutable request-details owner. Explicit quantity/package/brand intent remains separate from planner arithmetic, survives exact-request lifecycle restore, is pruned when recognized item keys disappear, and malformed payloads fail closed without altering the plan. Home delegates all request-details semantics to shared-core; no planner, ranking, pricing, evidence, clock, persistence, networking, or View authority moved into the new state owner. The provenance proof accepts both `candidate/*` and historical `work/valuepilot-*` candidates but excludes the milestone branch. Full candidate Android/browser/privacy/signature/release checks and milestone provenance passed.

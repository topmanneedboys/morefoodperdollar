...
The promoted milestone is now `ba1effcc34e0acefecedd2e4aed7dfc534324d59` (`Add review-first price photo import to Compare Here`). Candidate workflow **33840519086** and milestone provenance workflow **33840982046** passed for the exact same SHA. Compare Here now supports a bounded user-triggered local price-photo import: the image is decoded on-device at a bounded dimension, OCR output is capped and filtered, suggestions are appended into the existing editor without truncation or automatic comparison, and the user must still review entries and confirm like-for-like alternatives. Image bytes are not persisted; only the existing bounded text draft can be saved.

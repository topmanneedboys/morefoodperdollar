...
The promoted milestone is now `b87f93f724f533f8181cdba70418957ae9b20126` (`Clarify offline catalog misses are not availability`), verified by candidate build workflow **33823219836** and milestone provenance workflow **33823592680**. The bundled GTA and Metro Vancouver identity snapshots contain 3,000 records each, 2,543 unique canonical identity names (`WITHIN_TARGET` for the 1,500–5,000 launch band), 2,960 grocery-hinted records, and 41 household-hinted records protected by the deterministic 10% household reserve; current offers remain explicitly `0` / `NOT_INCLUDED`. Home’s offline catalog handoff states that selecting a bundled identity name only edits the Home list; because the planner is still a tiny fictional sample vocabulary, the inserted name may remain unresolved. An empty catalog result now explicitly says that a miss in this snapshot does not mean the product is unavailable. The existing identity-only, no-price/no-availability disclosure remains intact, and catalog selections remain query-only until the separate exact-identity confirmation boundary is deliberately wired. The weekly selector recognizes household category branches plus conservative, context-checked household name hints, keeps that bounded reserve, and persists selection measurements into the atomic `coverage-report.json`; false-positive guards are covered by deterministic tests. These are identity/category measurements only, not demand, retailer availability, stock, price, package quantity, freshness, or ranking authority. The merchant-feed qualifier still reports identity coverage separately from structural current-offer and unit-value candidate counts; those latter measurements remain `STRUCTURAL_ONLY` with authority `NONE`. The Android build workflow includes both qualifier paths in its push/PR filters. This adds no prices, offers, availability, planner/ranking authority, Android networking, or any new identity claim. Signed manifests and promotion pointers remain authoritative, and rejected regressions preserve the prior report and last-known-good pointers. Clean-source verification passed 400 shared-core tests and 1,337 Android app tests with zero failures, all 58 Android tasks, 64 Python tests, 30 browser tests, Firefox lint with zero findings, APK privacy inspection with no INTERNET/ACCESS_NETWORK_STATE, one-signer verification, exact-SHA release-bundle provenance, and exact-SHA offline-catalog workflows **33822329279** (previous candidate) and **33822724096** (previous milestone).

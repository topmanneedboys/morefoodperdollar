...
The promoted milestone is now `779d3b7193af198ae55c97b3e8e818443e08ccf2` (`Keep Home private-history recovery actionable`). Candidate workflow **33868976333** and milestone provenance workflow **33869478881** completed successfully for the exact same SHA. When Home cannot read the device-only comparison store, it keeps the typed `Review private price history` action visible so the shopper can open Scan & compare and use its existing recovery/clear controls. Home still suppresses every unreadable history-derived row and summary, and the action carries no price, offer, store, availability, planner, ranking, network, provider-economics or persistence authority. Focused tests pass. The full Windows Android test task still reports the repository's known line-ending-sensitive source-boundary failures; Linux candidate Android verification, lint/build, privacy, signing, release bundle, browser checks and the Python/catalog suite are green.

...
The promoted milestone is now `5932449f2c09e72fb677a1e79bf05556fbfcae50` (`Disclose unavailable private history on Home`). Candidate workflow **33856774432** and milestone provenance workflow **33857251205** passed for the exact same SHA. Home now carries an explicit typed availability status for its device-only comparison history: an unreadable or invalid store is disclosed in an accessible recovery notice, and history-derived row context is suppressed until the store is readable again. Empty history remains distinct from unavailable history, while the existing fictional planner result and unknown states pass through unchanged. The dedicated single-product “Is this a good price?” route still accepts one bounded manual entry, delegates parsing, selected-price semantics and exact unit math to the existing Compare Here bridge, and presents a private-history answer only: no history, last-price comparison, or personal range. Exact currency, package quantity, current/member basis and exact promotion facts are required; unknown or conflicting facts do not become a recommendation. A successful user check emits a source-labelled `CONFIRMED_GOOD_PRICE_CHECK` entry into the same capped, atomic, integrity-checked device memory, while preserving source provenance and never feeding planner/ranking/current-offer authority. If private history cannot be read, the dedicated screen also says history is unavailable and offers local recovery instead of silently presenting an empty-history answer. Home’s action and the dedicated screen remain owner-gated, offline, private, and explicit that they are not live store pricing. The earlier confirmed Compare Here memory, review-first photo import, fictional sample disclosure, 30,000-record identity rail and atomic catalog release remain unchanged.

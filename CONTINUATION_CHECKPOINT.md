...
The current promoted baseline is `c6c36416ab136473d7ddc22eef232de6ed090e55` (`Bound Practical Shopping Home query input`), verified by candidate workflow **33631294443** and promoted workflow **33631859937**. Home supports typed removal of resolved items and unresolved tokens, immediate deterministic replanning, preserved unknown/ambiguous states, item-specific accessibility descriptions, deterministic `and`/`&` list syntax, and an explicit “Price freshness” label on result evidence summaries. Its advanced extra-stop preference is progressively disclosed only after an actual result; immutable presentation owns availability, the View binds it mechanically, and the persisted exact threshold still governs the first plan and later replanning. The Home list editor now exposes the existing 240-character policy as a visible counter and accepts at most the model's existing 241-character retained error sentinel. The immutable render state carries that bound; the View only installs the corresponding filter/counter and does not own query, planning or ranking policy. Compare Here supports bounded per-entry removal while preserving its minimum two-slot input shape, plus an explicit Current shelf prices / Member prices control wired through pure session state into the existing exact route. The primary Compare action is disabled until two nonblank entries and explicit like-for-like confirmation are present; readiness is pure/bounded and deliberately does not parse evidence, so exact validation and honest unknown/error outcomes remain in the existing route. The physical editor applies the evidence adapter's existing 4,096-character per-block bound before text can be retained or persisted. An oversized restored block becomes an explicit empty/error entry rather than being truncated into facts that might rank. Price-basis changes invalidate stale output, restore safely across lifecycle state, default legacy/unknown stored values to CURRENT, and preserve strict member semantics with no current-price fallback. Manual-entry guidance demonstrates the exact current/member labels understood by the parser, while insufficient-evidence guidance names the selected basis and explicitly rejects current-price substitution in member mode. Independent clean-LF Android/browser/privacy/signing verification also passed. The local worktree was clean at verification.

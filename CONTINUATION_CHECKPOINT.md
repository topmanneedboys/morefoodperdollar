...
`f4e260d721fc6768dbf0635f33a1ce2316b38162` (`Name Saved row removal actions`) is the promoted milestone, verified by candidate workflow **33681822419** and milestone workflow **33682397427**. Saved product and store rows now carry immutable, item-specific accessibility descriptions for their removal actions, while loading, refreshing, updating and error projections continue to suppress mutation actions and descriptions. The descriptions are renderer-ready copy; the View only binds them and emits the existing typed preference action. The preceding Watch policy refinement remains intact: each numeric editor announces its projected field and unit, repeated Apply actions identify their field, and the no-distance-limit choice names the distance rule it changes. Home item details remain explicitly preference-only and never change the fictional sample total. Clean-source verification passed 1,634 JVM tests (375 shared-core + 1,259 Android app), all 58 Android tasks, all 30 browser tests, APK privacy checks, one-signer verification and release-bundle provenance.

...
`b4af05b6a38d233fb3e8ec85e0d95f491ccab0c8` (`Disable repeat submit during home refinement`) is the promoted milestone, verified by candidate workflow **33704284840** and milestone provenance workflow **33704835479**. Home’s immutable readiness now disables the repeat Plan my shop action while a submitted list needs clarification, so refinement choices or unresolved-item correction are the sole forward actions until the list is actionable; editing the query or choosing a valid option returns the normal flow. Home’s owner-driven editor, submit, compare, item-removal, refinement and extra-stop controls also fail closed when their nullable callbacks are absent; the IME path rejects an unwired owner. Basket’s typed “open Home” control remains disabled until its owner callback is attached, so replaceable or detached surfaces cannot expose visible no-op navigation actions. Home and Basket still pass their already-visible fictional/offline sample disclosure into the shared result-card accessibility summaries, so focusing a projected primary or optional-second-stop card cannot make sample prices sound like live retailer offers. The shared renderer retains its accessibility-tree suppression for decorative child labels, and reusable callers may still use the one-argument renderer without sample context. Saved’s destructive “Clear all” action still requires an explicit confirmation dialog before emitting its existing typed lifecycle action; individual item removals remain direct. The Home item-preferences dialog still shows “Clear preferences” only when that item already has saved preferences. The existing no-coverage correction remains intact: the planner’s “not enough price coverage” result remains visible, the extra-stop rule is hidden because there is no primary store plan, and Basket labels the state “Price coverage needed” instead of presenting it as a usable basket plan. Basket and Saved shell copy remain state-safe, and complete/incomplete-primary behavior is unchanged. No planner, ranking, pricing, evidence, persistence, clock, provider or network authority moved into the View. Clean-source verification passed 1,649 JVM tests (375 shared-core + 1,274 Android app), all 58 Android tasks, all 30 browser tests, Firefox lint with zero findings, APK privacy checks with no network permissions, one-signer verification and release-bundle provenance.

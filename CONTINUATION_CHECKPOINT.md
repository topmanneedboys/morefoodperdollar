...
Latest promoted head: `34f70ee5b126696781a7eb2fb17dc62a051ef27a` (`Expose Home item preference details`), candidate workflow **33666621914**, milestone workflow **33667213656**. The current app slice adds a progressive per-item Home details surface around the existing typed `PracticalShoppingHomeSession.State`. Explicit package count, brand text and exact-product preference remain shopper intent separate from planner arithmetic, survive exact-request lifecycle restore, are pruned when recognized item keys disappear, and malformed payloads fail closed without altering the plan. The bounded editor delegates value construction to core types, while the Activity coordinates only the dialog and typed state transitions; no planner, ranking, pricing, evidence, clock, persistence, networking, or View authority moved into the new surface. The provenance proof accepts both `candidate/*` and historical `work/valuepilot-*` candidates but excludes the milestone branch. Full candidate Android/browser/privacy/signature/release checks and milestone provenance passed.

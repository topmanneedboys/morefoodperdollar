...
The promoted milestone is now `9b8b8b349586aeda0f71b5746be573d3221f12da` (`Remember confirmed Compare Here prices privately`). Candidate workflow **33843985987** and milestone provenance workflow **33844394607** passed for the exact same SHA. After the shopper explicitly confirms like-for-like alternatives and receives a fully READY exact comparison, the existing Compare Here route assembles a bounded device-only memory snapshot without reparsing or reconstructing facts from UI strings. It retains the selected current/member price basis, exact package quantity, exact unit rate, optional exact promotion terms, observed time and a deterministic SHA-256 fingerprint; atomic storage caps history at 256 entries, fails closed on corrupt/tampered documents, and exposes a dedicated clear action. No memory entry carries store, provider, stock, availability, public evidence or current-offer authority; it is never passed into organic ranking or the shared planner. Unconfirmed, incomplete, blocked, incompatible and restored-draft results remain non-persistent. The earlier review-first photo import, fictional sample-card disclosures, 30,000-record shared Canada identity rail and atomic catalog release remain unchanged.

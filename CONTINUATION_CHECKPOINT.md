...
The promoted milestone is now `482941bd0aa33ec72d93c60985a9dc753d13df78` (`Show personal price history insight after comparison`). Candidate workflow **33846053326** and milestone provenance workflow **33846493362** passed for the exact same SHA. After the shopper explicitly confirms like-for-like alternatives and receives a fully READY exact comparison, the existing Compare Here route assembles a bounded device-only memory snapshot without reparsing or reconstructing facts from UI strings. It retains the selected current/member price basis, exact package quantity, exact unit rate, optional exact promotion terms, observed time and a deterministic SHA-256 fingerprint; atomic storage caps history at 256 entries, fails closed on corrupt/tampered documents, and exposes a dedicated clear action. On the same successful action, Compare Here compares each exact entry only with matching private history. One prior observation is reported as below, equal to or above the last remembered rate; at least two prior observations produce a personal low-to-high range. No matching history is presented honestly rather than inferred as a “good” price. No memory entry carries store, provider, stock, availability, public evidence or current-offer authority; it is never passed into organic ranking or the shared planner. Unconfirmed, incomplete, blocked, incompatible and restored-draft results remain non-persistent. The earlier review-first photo import, fictional sample-card disclosures, 30,000-record shared Canada identity rail and atomic catalog release remain unchanged.

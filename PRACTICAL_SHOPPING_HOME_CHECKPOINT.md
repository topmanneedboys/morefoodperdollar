...
The latest promoted offline-catalog performance slice is `e9f4d92f6956afcb0ad88657662b783870ccb66b` (`Index offline barcodes for fast catalog lookup`). Candidate workflow **34049215506** passed for the exact SHA: Android job **101529681106** (403 shared-core tests, 1,591 Android tests, lint/build, privacy and single-signer), browser job **101529681273** (30 tests and Firefox package checks), and release-bundle job **101530391192**. Milestone workflow **34049547297** and provenance job **101530550909** verified the promoted ref and exact candidate artifact lineage (candidate artifact digest `sha256:30d078a3d7ec026b03c6663e9edd9a0ba1b3d9046fe48391e5856176fff91e07`). The shared bounded identity index precomputes canonical GTIN buckets at construction, making repeated barcode discovery across the 30,000-record offline catalog cheaper while preserving deterministic record-id collision ordering, candidate/result bounds, evaluated counts and the existing text matcher. The index remains identity-only: it does not assert price, package quantity, store, availability, evidence, planner, ranking, persistence, networking, demo-data or provider-economics facts. Local verification passed 403 shared-core tests, 1,591 Android tests, 80 Python/catalog tests and 30 browser tests with zero failures or skips; signed 30,000-record GTA/Metro Vancouver catalogs and the signed 6,093-location store directory verified. Physical-device barcode/camera, accessibility and latency review and lawful production Home activation remain open; provider feeds remain blocked pending explicit rights.

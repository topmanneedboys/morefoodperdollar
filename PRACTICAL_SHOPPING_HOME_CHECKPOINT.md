...
The latest promoted Home consumer slice is `0ddf560f4091de4575250d512190a68934aaf716` (`Put Home demo shortcuts after first-value actions`). Candidate workflow **34009409773** passed for the exact SHA, including release-bundle job **101423023413**; milestone workflow **34009644195** and provenance job **101423093708** verified the promoted ref and exact artifact lineage. The blank fictional Home demo now presents `Plan my shop` and `Quick answers` (`Scan & compare prices` / `Is this a good price?`) before its bounded `Try a sample item` shortcuts. This improves first-value hierarchy only: the shortcuts remain visibly fictional/offline, and no planner, product, offer, store, availability, evidence, ranking, persistence, networking, account, tracking, demo-data or provider-economics authority moved into Home. Local verification passed 402 shared-core tests, 1,576 Android tests, 80 Python/catalog tests and 30 browser tests with zero failures or skips; Android lint/build/privacy/single-signer, signed offline catalogs/store directory and candidate release-bundle checks passed. Physical-device confirmation and lawful production Home activation remain open; provider feeds remain blocked pending explicit rights.

...
Next engineering slice: add the provider-neutral production request coordinator that can assemble explicitly selected identities, signed location-directory rows, private observed-price evidence and travel facts into the existing production assembler without inventing current offers. Keep the visible Home sample planning separate until that state has concrete local evidence and travel inputs; unknown prices remain unknown, one-store-first stays explicit, and Views remain limited to immutable rendering and typed actions.

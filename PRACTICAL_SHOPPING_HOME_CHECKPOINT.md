...
The latest promoted offline-catalog performance slice is `88addb8be0eef615655c3c070e13ee05687d8f79` (`Bound offline catalog text matches`). Candidate workflow **34052030346** passed for the exact SHA: Android job **101537239030** (405 shared-core tests, 1,591 Android tests, lint/build, privacy and single-signer), browser job **101537238850** (30 tests and Firefox package checks), and release-bundle job **101538040583**. Milestone workflow **34052459113** and provenance job **101538386492** verified the promoted ref and exact candidate artifact lineage (candidate release artifact digest `sha256:f31c32fbd6ab6490d92fbeb40bd3321e9934ecbe5cf85acb9ccf93e03de425db`). The shared bounded identity index now caches each identity record's immutable searchable fields/tokens, computes the normalized query-token set once, and retains only the deterministic best 200 candidate prefix during a 30,000-record scan before producing the existing 24-result page. Existing match kinds, deterministic ordering, indexed barcode lookup, candidate/result bounds and evaluated counts remain unchanged. The index remains identity-only: it does not assert price, package quantity, store, availability, evidence, planner, ranking, persistence, networking, demo-data or provider-economics facts. Local verification passed 405 shared-core tests, 1,591 Android tests, 80 Python/catalog tests and 30 browser tests with zero failures or skips; signed 30,000-record GTA/Metro Vancouver catalogs and the signed 6,093-location store directory verified. Physical-device barcode/camera, accessibility and latency review and lawful production Home activation remain open; provider feeds remain blocked pending explicit rights.

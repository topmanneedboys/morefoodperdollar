...
The latest promoted Practical Shopping Home slice is code SHA `7b002bf5ee193ef122ef6b30c9c61230fd2b988c` (`Retain shared image while OCR is pending`), following `078e8b6946183e74ce1de26b4223e054f3aedcf1` (`Accept shared images for local comparison`). Candidate workflow **34060944264** passed for the exact SHA: Android job **101561238171** (405 shared-core tests, 1,606 Android tests, test/lint/build, privacy and single signer), browser job **101561238457** (30 tests and Firefox package checks), and release-bundle job **101562062418**. Candidate release artifact digest: `sha256:a60cabe2618ba3db935695965bd72e006d772c70e2c071729c5e5d2c8fa4df7a`. Milestone workflow **34061301327** and provenance job **101562207724** verified the promoted milestone ref and exact candidate artifact lineage (provenance-copy artifact digest `sha256:2e22e508359ed963ff2e06996ca1ea1e769cd6d84277f8c6a7ee62ff6bde65e0`). Share-to-ValuePilot now accepts a user-selected `content:` image URI, validates it locally, and hands it through the existing Compare Here photo/OCR review path with temporary read permission. The URI is transient instance state only while OCR is pending and is cleared on completion, cancellation, invalidation or finish; no image is uploaded, scraped, persisted as evidence or used for ranking. Image OCR remains ordinary editable, unconfirmed suggestions and the existing exact package, currency, price, promotion, like-for-like, evidence, private-memory and planner gates remain authoritative. Text sharing remains supported, while malformed or unsupported image inputs fail closed with a clear status. This slice preserves the fictional/demo separation and no Android networking boundary. Physical-device share/OCR, accessibility and latency review and lawful production Home activation remain open; provider feeds remain blocked pending explicit rights.

...
The authoritative promoted code head is `595a1a3c84fa477c4000e2b7d974ab1402d9540e` (`Make complete OCR details the primary add action`). Candidate workflow **34038698313** passed for the exact SHA, including Android privacy/single-signer checks and release-bundle job **101502119586**; milestone workflow **34039012070** and provenance job **101502231333** verified the promoted ref and exact candidate artifact lineage. In Scan & Compare's review-first photo flow, when every presented OCR row has a complete detected-details draft, `Add with detected details` is now the primary explicit action while `Add selected` remains an explicit names-only fallback; mixed or incomplete rows retain the conservative names-only primary with detected details optional. All OCR values remain ordinary editable, unconfirmed text, and the existing exact package-quantity, currency, price, promotion, like-for-like and evidence gates remain authoritative. This bounded consumer-friction correction keeps all result authority in the existing controllers and shared core: no planner, projector, parser, money/quantity, confirmation, evidence, private-memory, ranking, product, offer, store, availability, persistence, networking, account, tracking, demo-data or provider-economics authority moved into a View. Local verification passed 402 shared-core tests, 1,584 Android tests, 80 Python/catalog tests and 30 browser tests with zero failures or skips; Android lint/build/privacy/single-signer, signed offline catalogs/store directory and candidate release-bundle checks passed. Physical-device OCR review/accessibility behavior and lawful production Home activation remain open; provider feeds remain blocked pending explicit rights.

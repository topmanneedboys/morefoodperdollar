...
The authoritative promoted head is `f002ac108070611685eb2b09467cd4bd18f2c5ed` (`Add context to Home choice accessibility labels`). Candidate workflow **34014947116** passed for the exact SHA, including Android privacy/single-signer checks and release-bundle job **101437620124**; milestone workflow **34015221826** and provenance **101437702604** verified the promoted ref and exact candidate artifact lineage. Home's dynamic Chicken-refinement and progressive extra-stop choice chips now give TalkBack context-rich descriptions that name the question each choice answers, while preserving the existing checked state and typed owner actions. This is presentation/accessibility-only: no parser, money, quantity, confirmation, evidence, private-memory, planner, projector, ranking or provider authority moved into a View. Home, Basket, plan-result/share and Watch store-status wording remain unchanged. Local verification passed 402 shared-core tests, 1,580 Android tests, 80 Python/catalog tests and 30 browser tests with zero failures or skips; Android lint/build/privacy/single-signer, signed offline catalogs/store directory and candidate release-bundle checks passed. Physical-device Home accessibility/keyboard behavior and lawful production Home activation remain open; provider feeds remain blocked pending explicit rights.

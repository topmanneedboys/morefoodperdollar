...
The authoritative promoted code head is `477413d05aa0da1ac510daeac3116a847841f94d` (`Report barcode identity dialog cancellation`). Candidate workflow **34036737133** passed for the exact SHA, including Android privacy/single-signer checks and release-bundle job **101496845980**; milestone workflow **34037053012** and provenance **101496927816** verified the promoted ref and exact candidate artifact lineage. Compare Here and Good Price barcode identity-choice dialogs now report an explicit cancellation when dismissed before a name is accepted and clear the processing indicator, while committed choices retain their success status. Request-id, lifecycle and reset guards prevent stale callbacks from overwriting the outcome. This bounded status/presentation correction keeps all result authority in the existing controllers and shared core: no parser, exact money/quantity, confirmation, evidence, private-memory, planner, projector, ranking, product, offer, store, availability, persistence, networking, account, tracking, demo-data or provider-economics authority moved into a View. Local verification passed 402 shared-core tests, 1,580 Android tests, 80 Python/catalog tests and 30 browser tests with zero failures or skips; Android lint/build/privacy/single-signer, signed offline catalogs/store directory and candidate release-bundle checks passed. Physical-device barcode/cancellation accessibility behavior and lawful production Home activation remain open; provider feeds remain blocked pending explicit rights.

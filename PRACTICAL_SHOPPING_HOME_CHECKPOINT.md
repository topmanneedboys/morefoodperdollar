...
`bb35e07202eb116474d873ead3eb449fcff6aeba` — `Verify ValuePilot state records in release CI`

Candidate workflow **33905499546** and milestone provenance workflow **33906002490** passed for the exact SHA. The prior Home projection now exposes `Add observed price` only for explicit missing-price rows; the typed callback opens the local Good Price screen with a bounded item name as untrusted prefill, while exact package quantity and observed price remain required before private-memory capture. Blank or oversized external prefills are ignored, complete-price rows stay quiet, and teardown clears the callback. The release workflow now includes the durable state/checkpoint records in its push and pull-request path filters, so docs-only milestone updates receive the same candidate and exact-SHA provenance gates. The handoff does not establish product identity, store, live price, offer, availability, planner or ranking authority and leaves the fictional sample result unchanged. Candidate Linux Android/browser/privacy/signing/release and Python/catalog gates passed; the local Windows run retains the known 18 line-ending-sensitive boundary failures, while physical-device ergonomics and production Home wiring remain open.

...
`1d23138c8b0b3b5ab97baa6fba55df055daffe43` — `Add truthful Data Status surface`

Candidate workflow **33878830784** and milestone provenance workflow **33879410427** passed for the exact SHA. The shell now exposes a `Data status` action below `Sources & licences`. Its immutable presentation reports the bundled shared 30,000-record Canada-labelled identity rail, readable private-observation count or unavailable-history recovery, and separate explicit “not included” states for current offers, store directory, flyers and remote connectivity. It repeats that Home/Search planner examples are fictional sample data. The dialog is lifecycle-safe and descriptive only; no planner, ranking, price, offer, store, stock, availability, provider-economics, persistence or network authority moved into Home. Candidate Linux verification, Android tests/lint/build, privacy, signing, browser, Python/catalog and release-bundle checks passed; the Windows run retains the known 18 line-ending-sensitive boundary failures. Physical-device barcode ergonomics and a lawful signed store-directory snapshot remain open.

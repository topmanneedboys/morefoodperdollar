...
`ed9fc561b01dcdbb04c80bee965535aa8f81752d` — `Add first-class good price check`

Candidate workflow **33852574190** and milestone provenance workflow **33853040970** completed successfully for the exact SHA. Home now has a clearly secondary `Is this a good price?` action that opens a dedicated offline single-product check. The route reuses the existing deterministic manual parser/comparison projection and strict current/member selection; it presents exact price, normalized package quantity, unit rate and conservative personal-history context only. With no matching history it says evidence is insufficient; matching observations are described relative to the last price or personal range. A successful check stores one exact source-labelled private-memory entry (`CONFIRMED_GOOD_PRICE_CHECK`) in the existing bounded atomic store, and the source survives integrity-checked codec round trips. Missing member prices, estimated quantities, ambiguous currencies, overlong input and malformed facts remain blocked and are never remembered. The action and dedicated renderer are owner-gated and contain no planner, ranking, store, availability, network or provider economics authority. The existing Home fictional planner demo, neutral Compare Here history count, `Scan & compare prices` flow, local photo OCR review, and identity-only offline catalog remain unchanged.

...
`d3cda7ba61931da27a68ea2b7ef5436a5ec47cbe` — `Make Home private history reviewable`

Candidate workflow **33890198052** and milestone provenance workflow **33890775156** passed for the exact SHA. Home’s existing `Review private price history` action now presents a bounded, read-only projection of the existing private comparison memory. It groups only exact like-for-like observations (including package quantity, currency/fraction, price basis, promotion terms and unit), then shows latest/low/high unit rates, package, basis, source and UTC observation date; rows are capped at 32 with an omitted-count disclosure. Copy explicitly identifies the rows as personal observations—not live store prices, inventory, retailer offers or guarantees—and never invents a store. A positive action opens the existing Scan & compare route; empty and unavailable memory preserve their existing recovery behavior. This is reversible presentation/navigation work only: the shared planner/projector, exact money and evidence authority are untouched. Focused tests, full candidate Android/browser/privacy/signing/release verification and milestone provenance passed; the Windows run retains the known 18 line-ending-sensitive boundary failures, while physical-device ergonomics and production Home wiring remain open.

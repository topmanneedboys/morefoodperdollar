...
The latest promoted Home consumer slice is `5852e98d11fd973c019fa6f95038bb7be41525af` (`Make planned store wording availability-safe`). Candidate workflow **34011076151** passed for the exact SHA, including release-bundle job **101427448941**; milestone workflow **34011330030** and provenance job **101427580764** verified the promoted ref and exact candidate artifact lineage. Home, Basket, the shared plan-result cards, the future production Home row and the privacy-safe share card now present store assignments as planned stops and visibly repeat `Planned stop only — product availability is not confirmed.` This keeps the consumer boundary honest without moving planner/projector, money, evidence, private-memory, catalog, store-directory, ranking or provider authority into a View; the fictional demo remains explicitly disclosed. Local verification passed 402 shared-core tests, 1,576 Android tests, 80 Python/catalog tests and 30 browser tests with zero failures or skips; Android lint/build/privacy/single-signer, signed offline catalogs/store directory and candidate release-bundle checks passed. Physical-device confirmation and lawful production Home activation remain open; provider feeds remain blocked pending explicit rights.

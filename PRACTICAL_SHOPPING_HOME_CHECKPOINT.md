...
The latest promoted offline-catalog performance slice is `ca4b406129dc349e9abd20850df51afeeab5b69e` (`Cache offline catalog text search metadata`). Candidate workflow **34050652764** passed for the exact SHA: Android job **101533543104** (404 shared-core tests, 1,591 Android tests, lint/build, privacy and single-signer), browser job **101533542957** (30 tests and Firefox package checks), and release-bundle job **101534371803**. Milestone workflow **34051003706** and provenance job **101534498827** verified the promoted ref and exact candidate artifact lineage (candidate artifact digest `sha256:53a9f8608340b880ca5e952c74893fd982ab621b35a8e56182a668975ed8ed44`). The shared bounded identity index now caches each identity record's immutable searchable fields/tokens and computes the normalized query-token set once, making repeated name, brand and alias discovery across the 30,000-record offline catalog cheaper while preserving existing match kinds, deterministic ordering, indexed barcode lookup, candidate/result bounds and evaluated counts. The index remains identity-only: it does not assert price, package quantity, store, availability, evidence, planner, ranking, persistence, networking, demo-data or provider-economics facts. Local verification passed 404 shared-core tests, 1,591 Android tests, 80 Python/catalog tests and 30 browser tests with zero failures or skips; signed 30,000-record GTA/Metro Vancouver catalogs and the signed 6,093-location store directory verified. Physical-device barcode/camera, accessibility and latency review and lawful production Home activation remain open; provider feeds remain blocked pending explicit rights.

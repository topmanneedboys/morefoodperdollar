...
The latest promoted Home consumer slice is `e9ab24a36e8b23017c5798c493f7184db4f7d36c` (`Keep Search sample disclosure on result rows`), following the planned-stop disclosure at `5852e98d11fd973c019fa6f95038bb7be41525af` and Watch candidate-store disclosure at `91b7367a3139050c86b951f2101db48a1cc06936`. Candidate workflow **34012905784** passed for the exact SHA, including release-bundle job **101432346545**; milestone workflow **34013209539** and provenance job **101432471208** verified the promoted ref and exact candidate artifact lineage. Visible Search sample rows now repeat `Fictional sample data only — not live retailer prices or availability.` and use the same copy for accessibility, so sample prices remain clearly fictional after scrolling. Home, Basket, plan-result and share surfaces continue to present assignments as planned stops; Watch continues to present a candidate store with availability unconfirmed. This remains presentation-only and moves no planner/projector, money, evidence, private-memory, catalog, store-directory, ranking or provider authority into a View. Local verification passed 402 shared-core tests, 1,578 Android tests, 80 Python/catalog tests and 30 browser tests with zero failures or skips; Android lint/build/privacy/single-signer, signed offline catalogs/store directory and candidate release-bundle checks passed. Physical-device confirmation and lawful production Home activation remain open; provider feeds remain blocked pending explicit rights.

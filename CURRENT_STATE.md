...
The promoted milestone is now `1d23138c8b0b3b5ab97baa6fba55df055daffe43` (`Add truthful Data Status surface`). Candidate workflow **33878830784** and milestone provenance workflow **33879410427** passed for this exact SHA. The shell now exposes a `Data status` action that reports the bundled shared 30,000-record Canada-labelled identity rail, the count or fail-closed unreadable state of device-only private observations, and separate explicit boundaries for current offers (`0` authorized records), store locations, flyers and remote connectivity. It also reiterates that Home/Search examples are fictional sample data. The immutable presentation helper is descriptive only; it adds no product, package, price, store, stock, availability, planner, ranking, provider-economics, persistence or network authority. Dialog lifecycle is closed on memory refresh and Activity teardown. Candidate Linux verification, Android tests/lint/debug build, privacy, signing, browser, Python/catalog and release-bundle checks are green. The Windows run retains the known 18 line-ending-sensitive boundary failures; physical-device barcode ergonomics and a real store directory remain open.

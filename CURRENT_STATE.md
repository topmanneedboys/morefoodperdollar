...
`b415cb7b6e723bf3c52958fd62a9e651662623e1` (`Clarify basket without price coverage`) is the promoted milestone, verified by candidate workflow **33693965885** and milestone provenance workflow **33694455323**. Home and Basket now fail closed when a recognized request has no usable price coverage: the planner’s “not enough price coverage” result remains visible, the extra-stop rule is hidden because there is no primary store plan, and Basket labels the state “Price coverage needed” instead of presenting it as a usable basket plan. Basket also hides the generic check-off progress line until a primary store plan exists, while its guidance directs the shopper back to adjust the sample list. Existing complete and incomplete-primary behavior is unchanged, including typed check-off eligibility and the local-only check-off notice. No planner, ranking, pricing, evidence, persistence, clock, provider or network authority moved into the View. Clean-source verification passed 1,641 JVM tests (375 shared-core + 1,266 Android app), all 58 Android tasks, all 30 browser tests, Firefox lint with zero findings, APK privacy checks with no network permissions, one-signer verification and release-bundle provenance.
...
The Practical Shopping MVP fixture/controller, one-store planner/projector, Home/Basket/Saved surfaces, Watch setup/policy presentation, and required consumer correction flows are implemented at the promoted head. The latest slice keeps no-coverage results honest by hiding recommendation-only controls and generic check-off progress until a primary store plan exists, labeling Basket “Price coverage needed,” and giving guidance that matches the actual evidence state.

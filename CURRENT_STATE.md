...
`773549b6138e18bb5e876d4486584c6370e855e1` (`Hide extra-stop controls without a plan`) is the promoted milestone, verified by candidate workflow **33692769781** and milestone provenance workflow **33693409380**. Home and Basket now fail closed when a recognized request has no usable price coverage: the planner’s “not enough price coverage” result remains visible, but the extra-stop rule is hidden because there is no primary store plan, and Basket no longer says to review a full recommendation. Basket instead explains that there is no usable price coverage and directs the shopper back to adjust the sample list. Existing complete and incomplete-primary behavior is unchanged, including typed check-off eligibility and the local-only check-off notice. No planner, ranking, pricing, evidence, persistence, clock, provider or network authority moved into the View. Clean-source verification passed 1,641 JVM tests (375 shared-core + 1,266 Android app), all 58 Android tasks, all 30 browser tests, Firefox lint with zero findings, APK privacy checks with no network permissions, one-signer verification and release-bundle provenance.
...
The Practical Shopping MVP fixture/controller, one-store planner/projector, Home/Basket/Saved surfaces, Watch setup/policy presentation, and required consumer correction flows are implemented at the promoted head. The latest slice keeps no-coverage results honest by hiding recommendation-only controls until a primary store plan exists and giving Basket guidance that matches the actual evidence state.

...
The promoted milestone is `12f3d5b400b501280933093f0c2a199de1ba8934` (`Bind offline catalog manifests to selected region`), verified by candidate workflow **33807185013** and milestone provenance workflow **33807942372**. The offline catalog now has an explicit-input, deterministic, offline coordinator for the supported Greater Toronto Area and Metro Vancouver identity snapshots, and the Android asset boundary rejects a manifest whose declared region differs from the selected metro. It requires source/rights/timestamp/signing inputs, keeps identity data separate from current offers, verifies every regional candidate before promotion, and preserves existing pointers on coverage regression; it does not add networking or alter planner/ranking authority. Clean-source verification passed 397 shared-core tests and 1,333 Android app tests with zero failures, all 58 Android tasks, 61 Python tests, 30 browser tests, Firefox lint with zero findings, APK privacy inspection with no INTERNET/ACCESS_NETWORK_STATE, one-signer verification and exact-SHA release-bundle provenance. Historical superseded entries follow.

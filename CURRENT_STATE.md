...
`49260d1ab403020181fc4463f60f04592083ef2e` (`Announce Home status changes to assistive technology`) is the promoted milestone, verified by candidate workflow **33706125568** and milestone provenance workflow **33706720628**. Home’s changing refinement and validation feedback is now a polite accessibility live region, so TalkBack can hear messages such as the chicken clarification or over-limit correction after the user acts. The change is presentation-only and leaves immutable state, typed actions, and all existing fail-closed controls unchanged. The preceding Basket draft clarity remains intact: a ready-but-unsubmitted list says “Plan this list on Home,” while unresolved, over-limit and empty states retain their attention guidance. The fictional/offline sample disclosure, accessibility summaries, Saved confirmation, item-preference gating, no-coverage correction, and state-safe Basket/Saved shell copy remain intact. No planner, ranking, pricing, evidence, persistence, clock, provider or network authority moved into the View. Clean-source verification passed 1,651 JVM tests (375 shared-core + 1,276 Android app), all 58 Android tasks, all 30 browser tests, Firefox lint with zero findings, APK privacy checks with no network permissions, one-signer verification and release-bundle provenance.

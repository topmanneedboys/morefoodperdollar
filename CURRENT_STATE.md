...
`85a61be036a0c5ff1d5f73365621ccc58ba53916` (`Fail closed Home query wrapper without an owner`) is the promoted milestone, verified by candidate workflow **33709276461** and milestone provenance workflow **33709801226**. Home’s query editor now gates the `TextInputLayout` wrapper as well as the editor itself, so the built-in clear-text end icon cannot remain active when no owner callback exists. The change is presentation-only and leaves immutable Home state, typed actions, and all existing fail-closed controls unchanged. Search’s changing ready/loading/results/error status, Basket’s local check-off progress, Saved lifecycle status and Home refinement feedback remain announced through their existing live regions. Basket still distinguishes a ready-but-unsubmitted list from an incomplete one with “Plan this list on Home.” The fictional/offline sample disclosure, accessibility summaries, Saved confirmation, item-preference gating, no-coverage correction, and state-safe Basket/Saved shell copy remain intact. No planner, ranking, pricing, evidence, persistence, clock, provider or network authority moved into the View. Clean-source verification passed 1,655 JVM tests (375 shared-core + 1,280 Android app), all 58 Android tasks, all 30 browser tests, Firefox lint with zero findings, APK privacy checks with no network permissions, one-signer verification and release-bundle provenance.

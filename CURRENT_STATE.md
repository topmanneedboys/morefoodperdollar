...
The promoted milestone is now `ba1effcc34e0acefecedd2e4aed7dfc534324d59` (`Add review-first price photo import to Compare Here`). Candidate workflow **33840519086** and milestone provenance workflow **33840982046** passed for this exact SHA. The existing Compare Here path now offers a user-triggered local image import: bounded decoding and on-device OCR add only reviewable text suggestions, never auto-compare, never persist image bytes, and never bypass the existing like-for-like confirmation or exact route coordinator. OCR results are capped and reject overlong/control-text snippets without truncation. The Home comparison entry is labelled “Scan & compare prices” while the optional Accessibility scanner remains separate.

...
`362e3960e1cc724b4791a5a8f2c2c4eb797dc81b` (`Clarify unsubmitted basket draft state`) is the promoted milestone, verified by candidate workflow **33705066985** and milestone provenance workflow **33705664237**. Basket now distinguishes a typed-but-unsubmitted Home list from an actually incomplete list: the ready draft says “Plan this list on Home,” explains that planning starts from Home, and keeps the collection action disabled; unresolved, over-limit and empty states retain their existing attention guidance. This is renderer-only copy derived from immutable source fields and does not duplicate the shared one-store planner/projector. Home’s immutable readiness still disables repeat submission while a submitted list needs clarification, and all owner-driven controls fail closed when callbacks are absent. The fictional/offline sample disclosure, accessibility summaries, Saved confirmation, item-preference gating, no-coverage correction, and state-safe Basket/Saved shell copy remain intact. No planner, ranking, pricing, evidence, persistence, clock, provider or network authority moved into the View. Clean-source verification passed 1,650 JVM tests (375 shared-core + 1,275 Android app), all 58 Android tasks, all 30 browser tests, Firefox lint with zero findings, APK privacy checks with no network permissions, one-signer verification and release-bundle provenance.

...
The promoted milestone is `5789448b3836d5d6fe8757209cf47fe0fe067181` (`Trigger full verification for merchant feed qualification`), verified by candidate workflow **33817795995** and milestone provenance workflow **33818227384**. Home’s offline catalog result explains that selecting a bundled identity name only edits the Home list and that the fictional sample planner may still leave that inserted name unresolved. The existing no-price/no-availability disclosure remains, and catalog choices stay query-only until the separate exact-identity confirmation boundary is deliberately wired. The merchant-feed qualifier now reports identity coverage separately from structural current-offer and unit-value candidates; those counts remain `STRUCTURAL_ONLY` with authority `NONE` and cannot become current-price, availability, or organic-ranking facts. The Android build workflow now includes the qualifier paths in its push/PR filters. This adds no prices, offers, availability, planner/ranking authority, Android networking, or new consumer claim. The weekly offline catalog coordinator still emits deterministic diagnostic coverage with current-offer coverage `0` / `NOT_INCLUDED`; signed manifests and promotion pointers remain authoritative, and rejected regressions preserve the prior report/pointers. Clean-source verification passed 400 shared-core tests and 1,337 Android app tests with zero failures, all 58 Android tasks, 62 Python tests, 30 browser tests, Firefox lint with zero findings, APK privacy inspection with no INTERNET/ACCESS_NETWORK_STATE, one-signer verification, exact-SHA release-bundle provenance, and exact-SHA merchant-qualifier workflows **33818313377** (candidate) and **33818315651** (milestone). Historical superseded entries follow.

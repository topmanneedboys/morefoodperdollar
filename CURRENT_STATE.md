...
Latest promoted engineering head: `227ed24059784989bc2ee38845181297f0430d0e` (`Show Home plan on Basket tab`), verified by candidate workflow **33635484179** and promoted workflow **33636063647**. Basket is no longer placeholder copy: it presents the recognized list, unresolved tokens, exact already-projected one-store/optional-second-stop result, and the selected exact extra-stop rule. Empty and attention-required states return the shopper to Home without inventing a result. Home and Basket share one physical result-card renderer, while the Basket presentation boundary performs no planning, ranking, money formatting, or missing-price inference. Incomplete results remain explicitly labelled known subtotals and the fictional/offline disclosure is repeated prominently. Clean-source verification passed 1,546 JVM tests, all 58 Android tasks, all 30 browser tests, Firefox lint, APK permission inspection, and signing inspection.

...
The promoted milestone is `b87f93f724f533f8181cdba70418957ae9b20126` (`Clarify offline catalog misses are not availability`), verified by candidate build workflow **33823219836** and milestone provenance workflow **33823592680**. The bundled GTA and Metro Vancouver identity snapshots contain 3,000 records each and 2,543 unique canonical identity names within the 1,500–5,000 target; 2,960 records receive a grocery hint and 41 receive a household hint under the deterministic 10% reserve. Current offers remain `0` / `NOT_INCLUDED`. Home’s offline catalog result explains that selecting a bundled identity name only edits the Home list and that the fictional sample planner may still leave that inserted name unresolved. Empty results now explicitly say that a miss in this snapshot does not mean the product is unavailable. The existing no-price/no-availability disclosure remains, and catalog choices stay query-only until the separate exact-identity confirmation boundary is deliberately wired. The weekly selector now recognizes household category branches plus conservative, context-checked household name hints, persists bounded selection measurements into `coverage-report.json`, and rejects generic false-positive examples deterministically. These do not establish demand, retailer availability, stock, price, package quantity, freshness, or organic-ranking authority. The merchant-feed qualifier still reports identity coverage separately from structural current-offer and unit-value candidates; those counts remain `STRUCTURAL_ONLY` with authority `NONE`. The Android build workflow includes the qualifier paths in its push/PR filters. This adds no prices, offers, availability, planner/ranking authority, Android networking, or new consumer claim. Signed manifests and promotion pointers remain authoritative, and rejected regressions preserve the prior report/pointers. Clean-source verification passed 400 shared-core tests and 1,337 Android app tests with zero failures, all 58 Android tasks, 64 Python tests, 30 browser tests, Firefox lint with no findings, APK privacy inspection with no INTERNET/ACCESS_NETWORK_STATE, one-signer verification, exact-SHA release-bundle provenance, and exact-SHA offline-catalog workflows **33822329279** (previous candidate) and **33822724096** (previous milestone). Historical superseded entries follow.

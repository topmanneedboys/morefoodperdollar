...
The promoted milestone is `a036f3799f95f05ec6ed142ffc2bb7cca87ad3a4` (`Add deterministic offline catalog refresh pipeline`), verified by candidate workflow **33805171665**, dedicated catalog workflow **33805171629**, and milestone provenance workflow **33805935576**. The offline catalog now has an explicit-input, deterministic, offline coordinator for the supported Greater Toronto Area and Metro Vancouver identity snapshots. It requires source/rights/timestamp/signing inputs, keeps identity data separate from current offers, verifies every regional candidate before promotion, and preserves existing pointers on coverage regression; it does not add networking or alter planner/ranking authority. Clean-source verification passed 397 shared-core tests and 1,333 Android app tests with zero failures, all 58 Android tasks, 61 Python tests, 30 browser tests, Firefox lint with zero findings, APK privacy inspection with no INTERNET/ACCESS_NETWORK_STATE, one-signer verification and exact-SHA release-bundle provenance. Historical superseded entries follow.

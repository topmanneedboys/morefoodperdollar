...
`7714a4c8ffc06b3d5ebd2b4b8b234bea817dc70c` (`Explain local basket check-off scope`) is the promoted milestone, verified by candidate workflow **33691649928** and milestone provenance workflow **33692091233**. Basket’s existing typed check-off capability now includes a renderer-ready notice that check-off is only a local shopping-session aid: it does not place an order or change the plan. The notice appears only when the immutable projection says collection is enabled, while incomplete plans still keep missing-price rows visible, unassigned and explicitly “not ready to collect”; complete plans retain full collection. The preceding Home extra-stop accessibility disclosure, per-item store assignment, Home/Basket card summaries, check-off context, Watch selection descriptions, Saved removal actions and Watch policy field labels remain intact. No planner, ranking, pricing, evidence, persistence, clock, provider or network authority moved into the View. Clean-source verification passed 1,639 JVM tests (375 shared-core + 1,264 Android app), all 58 Android tasks, all 30 browser tests, Firefox lint with zero findings, APK privacy checks with no network permissions, one-signer verification and release-bundle provenance.
...
The Practical Shopping MVP fixture/controller, one-store planner/projector, Home/Basket/Saved surfaces, Watch setup/policy presentation, and required consumer correction flows are implemented at the promoted head. The latest slice makes Basket’s local-only check-off scope explicit without changing the existing plan, collection capability set, or shopping decision boundaries.

...
The promoted milestone is now `f1ea903b5400242b20c73b1ae683c9f226bf2bfa` (`Add user initiated camera capture to price comparison`). Candidate workflow **33859783985** and milestone provenance workflow **33860267357** passed for this exact SHA. Scan & Compare now offers a user-triggered `Take a price photo` action alongside the existing photo import. It requests CAMERA only after that tap, writes the captured image to a cache-scoped FileProvider URI with explicit temporary read/write grants, and sends the bounded image through the existing on-device OCR suggestion/review route. Import and capture actions share one in-flight gate, stale/lifecycle callbacks are ignored, temporary files are deleted on success, failure, cancellation or teardown, and camera-unavailable/permission-denied/error states leave existing entries unchanged. OCR status is an accessible polite live region. The existing Compare Here history, Home private-history availability disclosure, dedicated good-price check, fictional sample disclosures, 30,000-record shared Canada identity catalog and atomic catalog-release pointer remain intact. No camera permission is requested at launch; no network, live-price, stock, availability, planner or ranking authority was added.

...
The promoted milestone is now `d3cda7ba61931da27a68ea2b7ef5436a5ec47cbe` (`Make Home private history reviewable`). Candidate workflow **33890198052** and milestone provenance workflow **33890775156** passed for this exact SHA. Home’s existing `Review private price history` action now opens a bounded, read-only summary of the existing device-only comparison memory: exact-package histories are grouped deterministically and show the latest observation, low/high observed unit rates, package and price basis, source label and UTC observation date, with a 32-row cap and omitted-count disclosure. The summary explicitly says these are personal observations, not live store prices, inventory, retailer offers or guarantees; no store is inferred. It can route to the existing Scan & compare surface, while unavailable or empty memory preserves the prior recovery path. This is presentation-only: no planner/projector, money arithmetic, offer/store/availability, network, persistence or provider-economics authority changed. Local Python/browser/Android focused and full build/privacy/signing checks passed; the Windows run retains the known 18 line-ending-sensitive boundary failures and physical-device ergonomics/production Home wiring remain open.

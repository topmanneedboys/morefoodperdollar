...
The promoted milestone is `52211c9a68e7c7537c872f5d62a48513de2d5122` (`Make offline Home catalog matches actionable`), verified by candidate workflow **33811804791** and milestone provenance workflow **33812349630**. Home’s offline catalog result dialog now exposes bounded, explicit name choices: selecting one replaces only the first whole unresolved query token, then returns the list for shopper review before Plan My Shop. Invalid, unsafe, or overlong edits fail closed. This never confirms an exact product, submits a plan, adds price/stock/availability, or changes planner/ranking authority. The catalog remains identity-only Canada-labelled Open Food Facts metadata across the supported Greater Toronto Area and Metro Vancouver snapshots, with deterministic deduplication, conflict omission, bounds, rights/timestamp/signature admission, and regional fail-closed behavior. Clean-source verification passed 400 shared-core tests and 1,337 Android app tests with zero failures, all 58 Android tasks, 61 Python tests, 30 browser tests, Firefox lint with zero findings, APK privacy inspection with no INTERNET/ACCESS_NETWORK_STATE, one-signer verification and exact-SHA release-bundle provenance. Historical superseded entries follow.

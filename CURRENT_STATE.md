...
The promoted milestone is now `779d3b7193af198ae55c97b3e8e818443e08ccf2` (`Keep Home private-history recovery actionable`). Candidate workflow **33868976333** and milestone provenance workflow **33869478881** completed successfully for the exact same SHA. When Home cannot read the device-only comparison store, it keeps the existing typed `Review private price history` action visible, allowing the shopper to open Scan & compare and use its recovery/clear controls. Home still hides all unreadable history rows and summaries. The full Windows Android test task retains the repository's known line-ending-sensitive source-boundary failures; Linux candidate Android verification, lint/build, privacy, signing, release bundle, browser checks and Python/catalog tests are green.

...
The promoted milestone is `b6018ab9d08f22558f01c0fe36609dad832b22f5` (`Search offline identity snapshots across supported metros`), verified by candidate workflow **33809477765** and milestone provenance workflow **33809973694**. The offline catalog now has an explicit-input, deterministic, offline coordinator for the supported Greater Toronto Area and Metro Vancouver identity snapshots, and Home searches both independently region-bound snapshots through a bounded shared discovery index. Identical identities are deduplicated, conflicting record ids are omitted, and the combined lookup fails closed if either bundled region is not admitted. It requires source/rights/timestamp/signing inputs, keeps identity data separate from current offers, verifies every regional candidate before promotion, and preserves existing pointers on coverage regression; it does not add networking or alter planner/ranking authority. Clean-source verification passed 397 shared-core tests and 1,333 Android app tests with zero failures, all 58 Android tasks, 61 Python tests, 30 browser tests, Firefox lint with zero findings, APK privacy inspection with no INTERNET/ACCESS_NETWORK_STATE, one-signer verification and exact-SHA release-bundle provenance. Historical superseded entries follow.

...
The promoted milestone is now `642e8666e144dbd0bdb4a65d765eb3890aa759a0` (`Surface private comparison history on Home`). Candidate workflow **33848650056** and milestone provenance workflow **33849153306** passed for this exact SHA. A fully READY, explicitly like-for-like Compare Here action creates a bounded private comparison snapshot from the already accepted exact facts: selected price basis, exact package quantity, exact unit rate, promotion terms, observation time and a deterministic local fingerprint. Compare Here gives a conservative personal-price insight from prior matching snapshots: one observation is described only relative to the last remembered rate, while multiple observations expose a personal range; mismatched labels, packages, currencies, price bases or promotion terms remain “no matching history.” Home now surfaces a neutral per-item count when private comparison history has the same normalized display label, while explicitly warning that package/promotion details may differ and the text is not live store pricing. The snapshot is stored only in app-internal atomic storage, capped at 256 records, rejects corrupt/tampered data instead of partially loading it, and is removable with a dedicated “Clear private comparison memory” action. It has no store, provider, stock, availability or public/current-offer authority, and it is never sent to the planner or ranking engine. Incomplete, blocked, incompatible, unconfirmed and restored-draft results do not create memory. The existing review-first local photo import, self-contained fictional sample disclosures, 30,000-record shared Canada identity catalog and atomic catalog-release pointer remain intact.

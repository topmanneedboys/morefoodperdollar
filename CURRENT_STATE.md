...
`6a9aa9ceb04ef20c0bc9cca17ce8af9cb6583950` (`Announce asynchronous search status to accessibility services`) is the promoted milestone, verified by candidate workflow **33708436275** and milestone provenance workflow **33709061137**. Search’s changing ready/loading/results/error status is now a polite accessibility live region, so TalkBack can hear asynchronous local-search feedback without changing the existing controller, provider, or projection. The change is presentation-only and leaves immutable search state, typed actions, and all existing fail-closed controls unchanged. Basket’s local check-off progress, Saved lifecycle status and Home refinement feedback remain announced through their existing live regions, and Basket still distinguishes a ready-but-unsubmitted list from an incomplete one with “Plan this list on Home.” The fictional/offline sample disclosure, accessibility summaries, Saved confirmation, item-preference gating, no-coverage correction, and state-safe Basket/Saved shell copy remain intact. No planner, ranking, pricing, evidence, persistence, clock, provider or network authority moved into the View. Clean-source verification passed 1,655 JVM tests (375 shared-core + 1,280 Android app), all 58 Android tasks, all 30 browser tests, Firefox lint with zero findings, APK privacy checks with no network permissions, one-signer verification and release-bundle provenance.

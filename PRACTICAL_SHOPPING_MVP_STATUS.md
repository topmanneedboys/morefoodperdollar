...
`9304aa29bccf06e3df8adf48d5f8943af838eb68` — `Announce Basket check-off progress to assistive technology`

GitHub Actions candidate workflow **33707613303** completed successfully, and milestone provenance workflow **33708102326** completed successfully for the exact same SHA.

The latest Basket refinement closes an accessibility feedback gap. Basket’s projected local check-off progress is now a polite accessibility live region, so assistive technology can hear the “n of m items collected” update after each typed foreground action or reset. This is a physical presentation binding only; the renderer still consumes immutable state, emits only existing typed actions, and owns no shopping or persistence authority. Clean-source verification passed 1,653 JVM tests (375 shared-core + 1,278 Android app), all 58 Android tasks, all 30 browser tests, Firefox lint with zero findings, APK privacy checks with no network permissions, one-signer verification and release-bundle provenance.

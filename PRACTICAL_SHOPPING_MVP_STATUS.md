...
`34f70ee5b126696781a7eb2fb17dc62a051ef27a` — `Expose Home item preference details`

GitHub Actions workflow run **33667213656** completed successfully (candidate run **33666621914** also passed).

The latest app-level slice adds a progressive per-item Home details surface around the existing typed `PracticalShoppingHomeSession.State`. Each resolved item now exposes an immutable summary and an Add/Edit details action. The bounded editor accepts explicit package count, preferred brand text and an exact-product shopper constraint, preserves undisplayed quantity dimensions, and makes the fictional-plan/no-arithmetic boundary explicit. Details restore only for the exact established request, are pruned when recognized item keys disappear during removal or replanning, and fail closed on malformed lifecycle payloads. The Home state owner and editor delegate value semantics to shared-core and carry no planner, ranking, price, quantity arithmetic, evidence, clock, persistence, networking, or View authority. The existing sample plan object is preserved by identity, so item intent cannot silently change basket totals or ranking. The milestone provenance check accepts the repository's historical `work/valuepilot-*` candidate branches while excluding the milestone branch itself.
...
Clean-source verification passed all 1,613 JVM tests (375 shared-core + 1,238 Android app) with zero failures/errors/skips, all 58 Android tasks, all 30 browser tests, Firefox packaging lint with zero findings, APK privacy inspection, and one-signer APK verification.

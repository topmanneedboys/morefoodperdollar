...
`6b37bb442e9c569690d74255262c4591bba6d2cc` — `Announce Compare Here readiness guidance`

GitHub Actions candidate workflow **33712739366** completed successfully, and milestone provenance workflow **33713227010** completed successfully for the exact same SHA.

The latest Compare Here refinement closes an accessibility feedback gap in the input/readiness surface. The projected “add products,” “confirm like-for-like,” and “ready” message block now announces politely as its immutable content changes, complementing the existing result-status live region and one-summary exact/blocked cards. This is a physical presentation binding only; the shell still consumes immutable state, emits only existing typed actions, and owns no shopping or persistence authority. Search result cards retain complete projected summaries with explicit fictional-sample context, and the shared projected Home/Basket result container remains a polite accessibility live region for newly planned recommendations. Clean-source verification passed 1,660 JVM tests (375 shared-core + 1,285 Android app), all 58 Android tasks, all 30 browser tests, Firefox lint with zero findings, APK privacy checks with no network permissions, one-signer verification and release-bundle provenance.

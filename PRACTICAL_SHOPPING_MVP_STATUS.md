...
`52e1f0648572143ad7acf7e68c5649fb01c38aa3` — `Announce Saved status changes to assistive technology`

GitHub Actions candidate workflow **33706915987** completed successfully, and milestone provenance workflow **33707458461** completed successfully for the exact same SHA.

The latest Saved refinement closes an accessibility feedback gap. Saved’s projected refresh, mutation, degradation and error status message is now a polite accessibility live region, so assistive technology can hear lifecycle feedback after a user action. This is a physical presentation binding only; the renderer still consumes immutable state, emits only existing typed actions, and owns no persistence or shopping authority. Clean-source verification passed 1,652 JVM tests (375 shared-core + 1,277 Android app), all 58 Android tasks, all 30 browser tests, Firefox lint with zero findings, APK privacy checks with no network permissions, one-signer verification and release-bundle provenance.

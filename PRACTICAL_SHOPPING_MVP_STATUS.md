...
`5ac18b0ae229135ca28f3882141ca30cb06948ca` — `Make Compare Here results accessible`

GitHub Actions candidate workflow **33711987021** completed successfully, and milestone provenance workflow **33712541943** completed successfully for the exact same SHA.

The latest Compare Here refinement closes an accessibility context gap. Its projected readiness/evaluation status is now a polite live region, and each exact or blocked result card exposes one coherent summary containing only the already-projected rank, product, price, quantity, unit rate and comparison blocker; decorative child labels are hidden so assistive technology does not announce the same fields repeatedly. This is a physical presentation binding only; the shell still consumes immutable state, emits only existing typed actions, and owns no shopping or persistence authority. Search result cards retain complete projected summaries with explicit fictional-sample context, and the shared projected Home/Basket result container remains a polite accessibility live region for newly planned recommendations. Clean-source verification passed 1,659 JVM tests (375 shared-core + 1,284 Android app), all 58 Android tasks, all 30 browser tests, Firefox lint with zero findings, APK privacy checks with no network permissions, one-signer verification and release-bundle provenance.

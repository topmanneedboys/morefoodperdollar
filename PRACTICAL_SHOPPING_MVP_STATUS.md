...
The promoted milestone is `642e8666e144dbd0bdb4a65d765eb3890aa759a0` (`Surface private comparison history on Home`), verified by candidate workflow **33848650056** and milestone provenance **33849153306**. Compare Here now turns a successful, explicitly confirmed exact comparison into bounded private device memory using the typed facts already accepted by the exact route, then immediately gives a conservative personal-price context when matching history exists. Each snapshot keeps selected price basis, exact quantity/rate, optional exact promotion terms, observed time and a deterministic fingerprint; it carries no store/provider/availability/current-offer authority and is excluded from planner/ranking inputs. A single prior observation is compared only with the last remembered rate; two or more prior observations define a displayed personal range. Exact label/package/currency/price-basis/promotion matching is required, and no matching history remains explicit rather than becoming a claim that a price is good. Home additionally shows a neutral label-only observation count for matching list items, with package/promotion mismatch and non-live caveats; it exposes no remembered price or planner input. Atomic local storage rejects invalid or tampered documents, caps history at 256 entries and provides an explicit clear action. Only READY user actions persist; blocked, incomplete, incompatible, unconfirmed and restored-draft states remain non-persistent.

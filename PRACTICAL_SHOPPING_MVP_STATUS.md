...
The promoted milestone is now `594876bc0e0eb414dafc5a69e9b91e76a9e85b21` (`Make missing Home prices actionable`), verified by candidate workflow **33903776239** and milestone provenance workflow **33904354573**. Home now gives an explicit missing-price row a renderer-owned `Add observed price` action. The typed handoff opens the existing local Good Price screen with only a bounded item name as an untrusted prefill; the shopper still enters exact package quantity and observed price before private-memory capture. Blank or oversized external prefills are ignored, complete-price rows do not expose the action, and Activity teardown clears its callback. This adds an actionable consumer path without changing the fictional sample result or inferring product identity, store, live price, offer, availability, planner, ranking, network or provider-economics facts. Candidate Linux Android tests/lint/build/privacy/single-signer/release, browser/Firefox and Python/catalog gates all passed; the Windows run retains the known 18 line-ending-sensitive boundary failures, physical-device ergonomics remain unverified, and production Home/private-price-book wiring remains unfinished.

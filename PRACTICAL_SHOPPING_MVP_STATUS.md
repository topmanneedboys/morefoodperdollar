...
The promoted milestone is now `bb35e07202eb116474d873ead3eb449fcff6aeba` (`Verify ValuePilot state records in release CI`), verified by candidate workflow **33905499546** and milestone provenance workflow **33906002490**. This preserves the explicit missing-price `Add observed price` Home action and adds a release-process guard that includes the durable state/checkpoint records in both workflow path filters, so docs-only milestone updates receive full candidate and exact-SHA provenance verification. The typed handoff opens the existing local Good Price screen with only a bounded item name as an untrusted prefill; the shopper still enters exact package quantity and observed price before private-memory capture. Blank or oversized external prefills are ignored, complete-price rows do not expose the action, and Activity teardown clears its callback. This adds an actionable consumer path without changing the fictional sample result or inferring product identity, store, live price, offer, availability, planner, ranking, network or provider-economics facts. Candidate Linux Android tests/lint/build/privacy/single-signer/release, browser/Firefox and Python/catalog gates all passed; the Windows run retains the known 18 line-ending-sensitive boundary failures, physical-device ergonomics remain unverified, and production Home/private-price-book wiring remains unfinished.

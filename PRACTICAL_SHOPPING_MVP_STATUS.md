...
The promoted milestone is now `d3cda7ba61931da27a68ea2b7ef5436a5ec47cbe` (`Make Home private history reviewable`), verified by candidate workflow **33890198052** and milestone provenance workflow **33890775156**. Home’s existing Review private price history action now opens a bounded read-only summary of exact, like-for-like device-only comparison histories, showing latest/low/high observed unit rates, package and price basis, source and UTC date, with a deterministic 32-row cap and omitted-count disclosure. The surface explicitly says the observations are personal and not live store pricing, inventory, retailer offers or guarantees; no store is inferred. Empty/unavailable memory keeps the existing recovery route, and the summary can open the existing Scan & compare screen. This reversible presentation/navigation slice adds no planner, ranking, price, offer, store, availability, network, persistence or provider-economics authority. Focused tests, candidate Android/browser/privacy/signing/release checks and milestone provenance passed; the Windows run retains the known 18 line-ending-sensitive boundary failures, physical-device ergonomics remain unverified, and production Home is still the fictional/demo path.

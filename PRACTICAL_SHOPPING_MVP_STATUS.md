...
The fictional fixture/controller and visible Home consumer path are complete and verified at the latest head. The newest Home slice makes missing-price rows actionable through a bounded, local Good Price handoff while preserving the sample/demo boundary. The requirements below are the historical acceptance criteria for that completed boundary; future work should define a separate typed item-detail/quantity contract or provider-neutral production orchestration boundary before changing planner authority.

Next engineering slice: define and regression-test the provider-neutral exact product/store/quantity/price handoff needed before production Home activation. Do not infer live offers from the identity catalog or location directory, and do not route this through the fictional controller.

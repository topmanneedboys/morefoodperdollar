...
The latest promoted code slice is `16a14798ed2ffd2b4471aa13381267b1cab76d77` (`Associate Compare Here editors with labels`). Candidate workflow **33995987426** passed for the exact SHA, and milestone provenance workflow **33996295982** verified the promoted ref and exact release artifact lineage. Compare Here’s dynamically-created editors now have generated runtime view IDs, with each visible `Product N` label linked to its editor via `labelFor` so TalkBack can announce the field context. These IDs are ephemeral view metadata only; they do not persist or represent product identity. The correction is presentation/accessibility-only and leaves existing exact parser, money/quantity, promotion, evidence, private-memory, planner/projector and ranking authority unchanged. No product, offer, store, availability, persistence, networking, account, tracking, demo-data or provider-economics boundary changed. Local verification passed 402 shared-core tests, 1,558 Android tests and 79 Python/catalog tests with zero failures or skips; browser verification passed 30 tests with Firefox lint clean. The signed 30,000-record GTA and Metro Vancouver identity snapshots and signed 6,093-record store directory also verified locally. Candidate Android lint/build, APK privacy, single-signer and release-bundle checks passed. Physical-device keyboard/accessibility ergonomics and lawful production Home activation remain open; provider feeds remain blocked pending explicit rights.

The latest promoted code slice is `571a53ac8bbb4ac1f99fa57bd77a5c6e3cd18568` (`Focus newly added Compare Here product input`). Candidate workflow **33994946204** passed for the exact SHA, and milestone provenance workflow **33995260282** verified the promoted ref and exact release artifact lineage. Compare Here now focuses the newly-created bounded editor block after an explicit `Add another product` action, places the cursor at the end and requests the existing keyboard helper. This removes a needless second tap for aisle-side entry without changing exact parsing, money/quantity, promotion, evidence, private-memory, planner/projector or ranking authority. No product, offer, store, availability, persistence, networking, account, tracking, demo-data or provider-economics boundary changed. Local verification passed 402 shared-core tests, 1,557 Android tests and 79 Python/catalog tests with zero failures or skips; browser verification passed 30 tests with Firefox lint clean. The signed 30,000-record GTA and Metro Vancouver identity snapshots and signed 6,093-record store directory also verified locally. Candidate Android lint/build, APK privacy, single-signer and release-bundle checks passed. Physical-device keyboard/accessibility ergonomics and lawful production Home activation remain open; provider feeds remain blocked pending explicit rights.

The latest promoted code slice is `e73b09454773e3b9b98a0f12a6277db33ca7ddea` (`Reveal Home plan after explicit actions`). Candidate workflow **33993225356** passed for the exact SHA, and milestone provenance workflow **33993536868** verified the promoted ref and exact release artifact lineage. After Home explicitly submits a list, resolves chicken refinement or replays Shop Again, the existing immutable projection is revealed through the parent scroll container with a bounded rectangle after layout. Passive typing, state restoration and private-memory refresh remain non-scrolling renders. This consumer ergonomics correction adds no planner/projector, exact money/quantity, evidence, product, offer, store, availability, ranking, persistence, networking, account, tracking, demo-data or provider-economics authority. Local verification passed 402 shared-core tests, 1,555 Android tests and 79 Python/catalog tests with zero failures or skips; browser verification passed 30 tests with Firefox lint clean. Candidate Android lint/build, APK privacy (no INTERNET or ACCESS_NETWORK_STATE), single-signer and checksummed release-bundle checks passed. Physical-device scroll/accessibility ergonomics and lawful production Home activation remain open; provider feeds remain blocked pending explicit rights.

The latest promoted code slice is `f4d8392fc3eb8da445d71dd4181cd3d5ac666539` (`Reduce product handoff friction safely`). Candidate workflow **33991590581** passed for the exact SHA, and milestone provenance workflow **33991889058** verified the promoted ref and exact release artifact lineage. The explicit Compare Here shared-text route now focuses and reopens the populated editable block. Home and Saved product-name routes focus and reopen Good Price after idle projection, while restored/recreated drafts are not force-focused. Good Price barcode identity choices now disclose replacement when an existing draft would be discarded, including a clear warning that package/price text is removed; the shopper must explicitly accept, and the barcode remains identity-only. This is a bounded consumer handoff correction: existing parser, exact money/quantity, evidence, private-memory, planner and ranking routes remain authoritative, with no product, offer, store, availability, persistence, networking, account, tracking, demo-data or provider-economics authority change. Local verification passed 402 shared-core tests, 1,553 Android tests and 79 Python/catalog tests with zero failures or skips; browser verification passed 30 tests with Firefox lint clean. Candidate Android lint/build, APK privacy (no INTERNET or ACCESS_NETWORK_STATE), one-signer and checksummed release-bundle checks passed. Physical-device keyboard/camera ergonomics and lawful production Home activation remain open; provider feeds remain blocked pending explicit rights.

The latest promoted code slice is `7cdd9d72c9aceb7daca366b7958664d52d3309b3` (`Reopen keyboard after identity handoff`). Candidate workflow **33990220399** passed for the exact SHA, and milestone provenance workflow **33990587690** verified the promoted ref and exact release artifact lineage. Scan & compare and Good Price now explicitly reopen the on-device keyboard after a reviewed OCR or barcode identity handoff focuses the editable field, removing an unnecessary second tap before the shopper enters package quantity and observed price. The handoff remains navigation-only: identity/OCR text is untrusted editable content, and the existing exact quantity, currency, price, promotion, like-for-like and private-memory routes remain authoritative. No planner, product, offer, store, availability, evidence, ranking, persistence, networking, account, tracking, demo-data or provider-economics authority changed. Local verification passed 402 shared-core tests, 1,550 Android tests and 79 Python/catalog tests with zero failures or skips; browser verification passed 30 tests with Firefox lint clean. Candidate Android lint/build, APK privacy (no INTERNET or ACCESS_NETWORK_STATE), one-signer and checksummed release-bundle checks passed. Physical-device keyboard/camera ergonomics and lawful production Home activation remain open; provider feeds remain blocked pending explicit rights.

The latest promoted code slice is `74e593fc4985a293501f8cd02e5dc4de8c6587e3` (`Share projected Home plans safely`). Candidate workflow **33989029339** passed for the exact SHA, and milestone provenance workflow **33989343043** verified the promoted ref and exact release artifact lineage. Home now gives a shopper an explicit `Share this plan` action after a projected primary store result. The preview and outgoing text are bounded, deterministic and privacy-safe: they use only existing projected total/store/coverage/travel/evidence and optional second-stop facts, omit item names and private history, and repeat the fictional/offline disclosure. No-coverage or unsafe/stale/owner-detached paths fail closed. This focused consumer presentation/lifecycle change adds no planner, projector, product, offer, store, availability, evidence, ranking, persistence, networking, account, tracking, demo-data or provider-economics authority. Local verification passed 402 shared-core tests, 1,548 Android tests and 79 Python/catalog tests with zero failures or skips; browser verification passed 30 tests with Firefox lint clean. Candidate Android lint/build, APK privacy (no INTERNET or ACCESS_NETWORK_STATE), single-signer, and checksummed release-bundle checks passed. Physical-device ergonomics/accessibility and lawful production Home activation remain open; provider feeds remain blocked pending explicit rights.

The latest promoted code slice is `22e7989e0146a9acfdb7dacf3480630103f0c4c9` (`Allow Good Price keyboard submission`). Candidate workflow **33987076763** passed for the exact SHA, and milestone provenance workflow **33987368167** verified the promoted ref and exact release artifact lineage. Good Price now accepts the keyboard `Done` action only when its existing visible check gate is enabled and sends it through the same exact route; result projection dismisses the keyboard and clears editor focus first. This is a bounded interaction correction with no parser, exact money/quantity, evidence, offer, store/availability, planner, ranking, persistence, networking, account, tracking, demo-data or provider-economics authority change. Local verification passed 402 shared-core tests, 1,541 Android tests, and 79 Python/catalog tests with zero failures or skips; browser verification passed 30 tests with Firefox lint clean. Physical-device ergonomics/accessibility and lawful production Home activation remain open; provider feeds remain blocked pending explicit rights.

The latest promoted code slice is `591ba936ee55373a3867084a4a930a69e40bb5ed` (`Keep exact price results visible after entry`). Candidate workflow **33986142487** passed for the exact SHA, and milestone provenance workflow **33986488417** verified the promoted ref and exact release artifact lineage. Compare Here and Good Price now dismiss the on-screen keyboard and clear editor focus before projecting their exact result, keeping the answer and any device-only memory outcome immediately visible. This bounded interaction correction changes no parser, exact money/quantity, evidence, offer, store/availability, planner, ranking, persistence, networking, account, tracking, demo-data or provider-economics authority; the existing routes remain the only result authorities. Local verification passed 402 shared-core tests, 1,540 Android tests, and 79 Python/catalog tests with zero failures or skips; browser verification passed 30 tests with Firefox lint clean. Physical-device ergonomics/accessibility and lawful production Home activation remain open; provider feeds remain blocked pending explicit rights.

The latest promoted code slice is `3864a339ded3c975f909789add947397e27dbc3b` (`Clarify fictional Shop Again disclosure`). Candidate workflow **33984116186** passed for the exact SHA, and milestone provenance workflow **33984866308** verified the promoted ref and exact release artifact lineage. Home's repeat-list action now says it reuses offline fictional demo data and is not a live retailer plan, so its standalone accessibility description cannot imply current retailer evidence. This presentation-only correction adds no planner, product, offer, store, availability, evidence, ranking, persistence, networking, account, tracking, demo-data or provider-economics authority. Local verification passed 402 shared-core tests, 1,538 Android tests, and 79 Python/catalog tests with zero failures or skips. Physical-device ergonomics/accessibility and lawful production Home activation remain open; provider feeds remain blocked pending explicit rights.

The latest promoted code slice is `445af5f80d5624e35af2c4c27cbd2b04516bcb18` (`Clarify projected savings as potential`). Candidate workflow **33981634428** passed for the exact SHA, and milestone provenance workflow **33982413282** verified the promoted ref at docs head `2a11bdde5863f1e565726b954f68b7ca34be0cb1` and exact release artifact lineage. The optional second-stop alternative now says `Could save ...` to make its projected nature explicit; exact savings math and eligibility remain planner/projector-owned. This presentation-only correction adds no product, offer, store, availability, evidence, ranking, persistence, networking, account, tracking, demo-data or provider-economics authority. Local verification passed 402 shared-core tests, 1,537 Android tests, and 79 Python/catalog tests with zero failures or skips. Physical-device ergonomics/accessibility and lawful production Home activation remain open; lawful provider feeds remain blocked pending explicit rights.

The latest promoted code milestone is `195aa654b05bea040a245ce454a06fa367deee76` (`Allow removing one private history observation`). Candidate workflow **33980306927** passed for the exact SHA, and milestone provenance workflow **33980621340** passed for the promoted ref and exact release artifact. Practical Shopping Home now exposes an explicit `Forget one observation` action only when readable, nonempty local comparison memory exists. The picker is deterministic and bounded to the 32 newest entries, carries exact observation fingerprints through an explicit confirmation, and re-reads the atomic store immediately before removing the selected row. Empty, corrupt, changed, erased, oversized, missing or failed mutations fail closed without deleting another entry. Individual deletion complements review, clear-all and export while preserving the private-memory boundary: no planner, projector, product, offer, store, availability, evidence, ranking, networking, account, tracking, demo-data or provider-economics authority moved. Candidate Linux verification passed 402 shared-core tests and 1,533 Android tests with zero failures or skips, lint/build/privacy/single-signer/release-bundle and browser/Firefox checks; local Python/catalog verification passed 79 tests. Physical-device removal/share and camera/OCR ergonomics plus lawful production Home activation remain open; lawful provider feeds remain blocked pending rights.

The latest promoted code milestone is `b94508453b4d51fbf192a9216d919b3b5818de9e` (`Add explicit private history export from Home`). Candidate workflow **33978834368** passed for the exact SHA, and milestone provenance workflow **33979149725** passed for the promoted ref and exact release artifact. Practical Shopping Home now exposes an explicit `Export private history` action only when the bounded local comparison memory is readable and nonempty. The pure, bounded exporter orders observations newest-first and includes exact price, package quantity, unit rate, price basis, promotion terms, UTC observation time and source label while clearly disclosing that the copy is personal historical context—not live retailer prices, inventory, availability or a guarantee—and omitting store claims because private memory has no store authority. A five-row preview and privacy warning precede a re-read of the atomic local store and Android text-share chooser; empty, corrupt, changed, erased or over-256-KiB histories fail closed without truncation, sharing or mutation. This is an ownership/convenience improvement only: no planner, projector, product, offer, store, availability, evidence, ranking, networking, account, tracking, demo-data or provider-economics authority moved, and the existing review/clear-all controls remain. Candidate Linux verification passed 402 shared-core tests and 1,533 Android tests with zero failures or skips, lint/build/privacy/single-signer/release-bundle and browser/Firefox checks; local Python/catalog verification passed 79 tests. Physical-device export/share and camera/OCR ergonomics plus lawful production Home activation remain open; lawful provider feeds are still blocked pending rights.

The latest promoted code milestone is `2bdf246c5a5843ff9f7384f4743c472e2b71dfda` (`Focus first OCR addition in Compare Here`). Candidate workflow **33976684845** passed for the exact SHA, and milestone provenance workflow **33977035497** verified the promoted ref and release artifact lineage. Scan & Compare now focuses the first editor block populated by an explicitly reviewed OCR addition; the bounded photo-draft result carries and validates that deterministic index. The previously promoted barcode path focuses its selected identity block, and Good Price focuses its manual product field after barcode identity selection. These changes only remove navigation friction after user actions. OCR/barcode content remains untrusted, exact package quantity/currency/price and like-for-like review remain mandatory, and no parser, product, offer, store, availability, evidence, planner, ranking, persistence, network, demo-data or provider-economics authority was added. Candidate Linux verification passed 402 shared-core tests and 1,529 Android tests with zero failures or skips, lint/build/privacy/single-signer/release-bundle and browser/Firefox checks; local Python/catalog verification passed 79 tests. Physical-device ergonomics/accessibility and lawful production Home activation remain open; `DEVICE_VALIDATION.md` is the unrun manual checklist.

The latest promoted code milestone is `06c37ae4e3c2834bb01dc73d7b85f7e6815c9cd0` (`Preserve Home refinements when shopping again`). Candidate workflow **33974689463** passed for the exact SHA, and milestone provenance workflow **33975020123** verified the promoted ref and release artifact lineage. A completed Home list can now be replayed from its existing model, retaining explicit refinements such as a selected chicken cut instead of losing them through a fresh query-draft transition. Existing typed request details remain reconciled to the stable replayed item keys. This is a bounded session/consumer-flow improvement; no planner, projector, price, evidence, offer, store, availability, ranking, persistence, network, demo-data or provider-economics authority was added. Candidate Linux verification passed 402 shared-core tests and 1,526 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle and browser/Firefox checks; local Python/catalog verification passed 79 tests. Physical-device ergonomics and lawful production Home activation remain open.

The latest promoted code milestone is `cfd4ba6663978dcc2f73447f218b0b4df643c568` (`Prefill complete OCR comparison drafts`). Candidate workflow **33973764156** passed for the exact SHA, and milestone provenance workflow **33974081700** verified the promoted ref and release artifact lineage. Scan & Compare's bounded photo-review dialog now gives the shopper an optional `Add with detected details` action for rows with one concrete ISO-currency price, exact non-derived quantity, usable name and parser-replayable promotion. The action copies only an editable ordinary-text proposal; ambiguous/multiple prices, unclear currency, estimated/derived quantities and unsupported promotions stay raw, and the exact manual adapter remains the only authority. The dialog/status continue to say the details are unconfirmed and require review before comparison or private-memory capture. The same correction reports skipped snippets once instead of double-counting them. No OCR, product, package, price, store, availability, evidence, planner, ranking, persistence, network, demo-data or provider-economics authority was added. Candidate Linux verification passed 402 shared-core tests and 1,525 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle and browser/Firefox checks; local Python/catalog verification passed 79 tests. Physical-device ergonomics and lawful production Home activation remain open.

The latest promoted code milestone is `55b1ac9c411444b5285fdcf6dc9740f3f8848cce` (`Explain OCR signals during photo review`). Candidate workflow **33972120095** passed for the exact SHA, and milestone provenance workflow **33972485006** verified the promoted ref and release artifact lineage. Photo review in Scan & compare now gives each bounded OCR candidate a deterministic, display-only summary of possible name, price text and package-size signals under an explicit `Review only` label. Multiple prices, ambiguous currency and estimated/missing quantity stay visible as review requirements. The exact route remains authoritative: the selected raw OCR block is appended only after explicit selection, then the shopper edits and confirms it before exact comparison or private-memory capture. No OCR, product, package, price, store, availability, evidence, planner, ranking, persistence, network, demo-data or provider-economics authority was added. Candidate Linux verification passed 402 shared-core tests and 1,522 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle and browser/Firefox checks; local Python/catalog verification passed 79 tests. Physical-device ergonomics and lawful production Home activation remain open.

The latest promoted code milestone is `8499e8ad2b566947d6bd6af70d8ab24b47cbf5c2` (`Open Home catalog identities in Compare Here`). Candidate workflow **33970594096** passed for the exact SHA, and milestone provenance workflow **33970918134** verified the promoted ref and release artifact lineage. A selected identity from Home's signed offline catalog lookup now has a clear `Use in Scan & compare` path when the original list word is unknown. Only the bounded identity display label crosses the existing untrusted text handoff; exact package quantity, currency, observed price and like-for-like review remain required in Scan & compare. Safe-label and dialog-generation gates reject malformed or stale actions, the exact product path is untouched, and no product/price/package/store/availability/evidence/planner/ranking/persistence/network/provider authority was added. The visible fictional Home remains demo-only. Candidate Linux verification passed 402 shared-core tests and 1,515 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle and browser/Firefox checks; local Python/catalog verification passed 79 tests. Physical-device ergonomics and lawful production Home activation remain open.

The latest promoted code milestone is `a231f0ecc962737f815dc2bfa9fe894037dd2bf9` (`Cancel stale Compare Here photo work`). Candidate workflow **33969229838** passed for the exact SHA, and milestone provenance workflow **33969555817** verified the promoted ref and release artifact lineage. Scan & Compare now exposes a bounded cancel action while local OCR is active. Draft edits, clear actions and Activity teardown invalidate the prior photo generation, reject late callbacks, remove temporary camera files and keep the current editor entries; no stale photo suggestion can be added. This focused lifecycle/ergonomics correction changes no parser, exact evaluator, money/quantity, private-memory, evidence, offer, store/availability, planner, ranking, persistence, Android networking, demo-data or provider-economics authority. Candidate Linux verification passed 402 shared-core tests and 1,515 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle and browser/Firefox checks; local Python/catalog verification passed 79 tests. Physical-device ergonomics and lawful production Home activation remain open.

The latest promoted code milestone is `9318a70d5b6a9a31ba0d4377efdfebd676ccf749` (`Restore Good Price results after recreation`). Candidate workflow **33967685757** passed for the exact SHA, and milestone provenance workflow **33967990673** verified the promoted ref and release artifact lineage. Good Price now restores its evaluated answer/share card after recreation by replaying the exact check with persistence disabled; its same-observation fingerprint is excluded from history, and any draft or memory change clears the replay marker. Memory failures remain fail-closed. This focused renderer/lifecycle correction changes no parser, exact evaluator, money/quantity, private-memory, evidence, offer, store/availability, planner, ranking, persistence, Android networking, demo-data or provider-economics authority. Candidate Linux verification passed 402 shared-core tests and 1,510 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle and browser/Firefox checks; local Python/catalog verification passed 79 tests. Physical-device ergonomics and lawful production Home activation remain open.

The latest promoted code milestone is `8c0f4e843b0c1cce266f17fcceda19f278b5711a` (`Preserve Good Price capture draft safely`). Candidate workflow **33966599555** passed for the exact SHA, and milestone provenance workflow **33966897180** verified the promoted ref and release artifact lineage. Good Price preserves its bounded typed product draft through recreation, does not replace a new intent prefill, and makes barcode cancellation explicit while preventing duplicate capture launches. Capture/lookup remains disabled only while its existing local work is active. This focused renderer/orchestration correction changes no parser, exact evaluator, money/quantity, private-memory, evidence, offer, store/availability, planner, ranking, persistence, Android networking, demo-data or provider-economics authority. Candidate Linux verification passed 402 shared-core tests and 1,508 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle and browser/Firefox checks; local Python/catalog verification passed 79 tests. Physical-device ergonomics and lawful production Home activation remain open.

The latest promoted code milestone is `9432604ca89572233e64b8268a0673ed35c6c0ce` (`Announce scanner availability accessibly`). Candidate workflow **33964877138** passed for the exact SHA, and milestone provenance workflow **33965190171** verified the promoted ref and release artifact lineage. The optional Compare Here scanner status is now a polite accessibility live region, making availability changes on resume perceivable without changing the existing scanner boundary or any shopping authority. This presentation-only correction adds no parser, money/quantity, evidence, offer, store/availability, planner, ranking, persistence or Android networking authority. Candidate Linux verification passed 402 shared-core tests and 1,505 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle and browser/Firefox checks; local Python/catalog verification passed 79 tests. Physical-device ergonomics and lawful production Home activation remain open.

The latest promoted code milestone is `60bd0358bc03a50c5180fea1e29a1ee3e65597cd` (`Invalidate stale private memory on resume`). Candidate workflow **33964012615** passed for the exact SHA, and milestone provenance workflow **33964271194** verified the promoted ref and release artifact lineage. Good Price now fails closed on resume when device-only private memory changes or becomes unreadable, clearing any projected answer/share card that could retain stale personal context; Compare Here similarly hides a saved-history status when paused storage no longer matches the state that produced it. This is lifecycle/presentation-only and adds no parser, money/quantity, evidence, offer, store/availability, planner, ranking, persistence or Android networking authority. Candidate Linux verification passed 402 shared-core tests and 1,505 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle and browser/Firefox checks; local Python/catalog verification passed 79 tests. Physical-device ergonomics and lawful production Home activation remain open.

The latest promoted code milestone is `d981fdcd81aa8101c3006f82b8b92744f3fd12ee` (`Announce private memory outcomes accessibly`). Candidate workflow **33963348099** passed for the exact SHA, and milestone provenance workflow **33963622041** verified the promoted ref and release artifact lineage. Good Price and Compare Here private-memory status changes now use polite accessibility live regions, preserving the existing exact routes and evidence boundaries while making saved, unavailable and clear-failure feedback announceable. Candidate Linux verification passed 402 shared-core tests and 1,504 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle and browser/Firefox checks; local Python/catalog verification passed 79 tests. Physical-device ergonomics and lawful production Home activation remain open.

The latest promoted code milestone is `d6a0b25ef3e20a7c533cb98fb083c910e734b641` (`Improve Good Price result accessibility`). Candidate workflow **33962568132** passed for the exact SHA, and milestone provenance workflow **33962826152** verified the promoted ref and release artifact lineage. Good Price now announces its exact projected answer and optional private-history context as coherent accessibility cards without repeating decorative labels. This is a presentation-only accessibility improvement; no parser, exact evaluator, money/quantity, memory, evidence, offer, store/availability, planner, ranking, persistence, Android networking, demo or provider-economics authority changed. Candidate Linux verification passed 402 shared-core tests and 1,503 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle and browser/Firefox checks; local Python/catalog verification passed 79 tests. Physical-device ergonomics and lawful production Home activation remain open.

The latest promoted code milestone is `44767d3a89c5fe79e3916c09ef91a589e42230dc` (`Invalidate Good Price result after memory clear`). Candidate workflow **33961384703** passed for the exact SHA, and milestone provenance workflow **33961699432** verified the promoted ref and release artifact lineage. Good Price now returns to its explicit idle state after private-history deletion succeeds, clearing the evaluated card and share action so removed personal context cannot remain visible or shareable. Failed deletion still reports an error without claiming success. No planner/projector, exact evaluator, memory-store authority, evidence, offer, store/availability, ranking, Android networking, demo-data or provider-economics authority changed. Candidate Linux verification passed 402 shared-core tests and 1,502 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle and browser/Firefox checks; local Python/catalog verification passed 79 tests. Physical-device ergonomics and lawful production Home activation remain open.

The latest promoted code milestone is `9c164ecd2566f26e9805cbcfce1356090cdf9ad6` (`Make photo retry outcomes explicit`). Candidate workflow **33960585727** passed for the exact SHA, and milestone provenance workflow **33960929862** verified the promoted ref and release artifact lineage. Scan & Compare now distinguishes recoverable photo failures from terminal decisions: OCR/no-match and recoverable picker/camera creation failures offer `Try another photo`, while permission denial, cancellation and unavailable camera hardware remain explicit no-retry states. This prevents a misleading loop after denied permission and adds recovery when a picker launch fails, without changing the existing review-first OCR proposal, exact parser, quantity/currency/price validation, like-for-like confirmation or private-memory authorities. No planner/projector, evidence, offer, store/availability, ranking, persistence, Android networking, demo-data or provider-economics authority changed. Candidate Linux verification passed 402 shared-core tests and 1,501 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle and browser/Firefox checks; local Python/catalog verification passed 79 tests. Physical-device ergonomics and lawful production Home activation remain open.

The latest promoted code milestone is `58c8732f1fe5dcb1a450fb9c0d28aa743f5dfb56` (`Add retry action for failed comparison photos`). Candidate workflow **33959540807** passed for the exact SHA, and milestone provenance workflow **33959812269** verified the promoted ref and release artifact lineage. Scan & Compare now offers a one-tap retry using the same user-selected camera/import method after a terminal OCR read failure or no-match result. The retry is fail-closed while capture/review is active, cleared after success or a new request, and leaves existing draft entries unchanged. It only improves recovery around the existing bounded on-device OCR proposal route; exact parser, quantity/currency/price validation, like-for-like confirmation, comparison and private-memory rules remain unchanged. No planner/projector, evidence, offer, store/availability, ranking, persistence, Android networking, demo-data or provider-economics authority changed. Candidate Linux verification passed 402 shared-core tests and 1,500 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle and browser/Firefox checks; local Python/catalog verification passed 79 tests. Physical-device ergonomics and lawful production Home activation remain open.

The latest promoted code milestone is `1db1b0d9edaa18e4b78ef11d08d317175d4e2902` (`Add repeat-list action to Practical Shopping Home`). Candidate workflow **33958293679** passed for the exact SHA, and milestone provenance workflow **33958570310** verified the promoted ref and release artifact lineage. Home’s completed nonempty result now includes an explicit `Shop again` action that re-runs the same typed list through the existing deterministic session/controller. It is hidden for idle, draft, refinement and error states, fail-closed while detached or before first render, and cleared on Activity teardown. No shared planner/projector, exact money/quantity, evidence, offer, store/availability, ranking, persistence, Android networking, demo-data or provider-economics authority changed. Candidate Linux verification passed 402 shared-core tests and 1,497 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle and browser/Firefox checks; local Python/catalog verification passed 79 tests. Physical-device ergonomics and lawful production Home activation remain open.

The latest promoted code milestone is `42f86e7dbd6a1e03a694c85032f30f21b1ab752e` (`Let saved products reopen Good Price`). Candidate workflow **33957008185** passed for the exact SHA, and milestone provenance workflow **33957294707** verified the promoted ref and release artifact lineage. Saved product rows now offer a typed `Check price` action that opens Good Price with a bounded display-name prefill only. Exact package quantity, currency and observed price remain required, and Good Price’s existing evaluator/private-memory path remains the sole authority; Saved identity never becomes current price, stock, availability or store evidence. Busy Saved lifecycle states suppress the convenience action and its route coordinator does not mutate Saved state. No planner/projector, money/quantity, evidence, offer, store/availability, persistence, Android networking, demo-data or provider-economics authority changed. Candidate Linux verification passed 402 shared-core tests and 1,494 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle and browser/Firefox checks; local Python/catalog verification passed 79 tests. Physical-device ergonomics and lawful production Home activation remain open.

The latest promoted code milestone is `37cc19cb72cb231831dfecef8fd5eb0c0e5bca2b` (`Add review gate for OCR comparison suggestions`). Candidate workflow **33955298400** passed for the exact SHA, and milestone provenance workflow **33955607275** verified the promoted ref and release artifact lineage. Photo/camera OCR suggestions now stop at a bounded, explicitly untrusted review dialog: the shopper selects which snippets belong in the comparison before any existing draft entry changes. The existing editable Compare Here path still owns parsing, exact quantity/price/currency validation, like-for-like confirmation, comparison and private-memory capture; cancellation, lifecycle invalidation, malformed/duplicate text and capacity limits leave existing entries unchanged. No planner/projector, evidence, offer, store/availability, persistence, Android networking, demo-data or provider-economics authority changed. Candidate Linux verification passed 402 shared-core tests and 1,490 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle, browser/Firefox and Python/catalog checks; physical-device camera/OCR ergonomics and lawful production Home activation remain separate.

The latest promoted code milestone is `0e1ffaaf24352ad9652631d84d081cd67db4699d` (`Clarify missing Home item price breakdowns`). Candidate workflow **33952630205** passed for that exact code SHA, and the resulting promoted tree was provenance-verified by milestone workflow **33953296196**. Home, Basket and the inactive production Home surface now expose the distinction between an exact per-item breakdown and a covered subtotal without optional line detail: exact values remain exact, while the latter says `Included in the basket total — exact item price not shown.` No planner/projector, money, evidence, offer, store/availability, ranking, networking, persistence or provider-economics authority changed, and missing/stale/conflicting/unknown states remain explicit. A test-only newline normalization helper preserves whole-file source-boundary assertions across Windows/Linux; the local full Android run now passes without the prior 18 newline-sensitive failures. Candidate Linux verification passed 402 shared-core tests and 1,485 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle, browser/Firefox (30 tests), and Python/catalog gates; physical-device ergonomics and lawful production Home activation remain separate.
...
The latest promoted code milestone is `730794ed111658383ede5350fededfd57c98ece9` (`Add generation-safe production Home handoff`), verified by candidate workflow **33930515512** for that exact SHA. Android now has a narrow production Home adapter that delegates all valid decisions to the existing `PracticalShoppingUiProjector`, and a bounded generation-aware host that re-evaluates the raw orchestration request against current lifecycle/disposition registries before handing only a sanitized immutable projection to its renderer. Stale, duplicate and same-generation-conflicting refreshes are rejected; structural production failures become an explicit unavailable state rather than a fake no-coverage result, while valid no-coverage stays visible. This prepares the production Home seam without activating Android networking, retailer feeds or the clearly labelled fictional Home controller. Candidate Linux verification passed 402 shared-core tests and 1,473 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle, browser/Firefox (30 tests) and Python/catalog (79 tests) gates; the local Windows run retains the known 18 line-ending-sensitive boundary failures, physical-device ergonomics remain unverified, and production Home activation remains a separate milestone.

The latest promoted code milestone is `3efdb688d5ff981b4f421bf349d49ef1432679e1` (`Add privacy-safe Good Price result sharing`), verified by candidate workflow **33927693684** and milestone provenance workflow **33928182141**. The first-class Good Price route now has an explicit, review-first share action for one exact evaluated result. The generic bounded card contains only the exact entered price, package quantity, selected price basis and unit-rate math, says it is based on a price entered rather than live store pricing, and excludes the product name, private history, receipts, location, account and source identifiers. Non-exact, blocked, ambiguous and unsafe states remain unshareable. This reversible consumer-clarity slice leaves the shared planner/projector, exact money/quantity authority, evidence, offers, stores, availability, persistence, Android networking, demo separation and provider-economics rules unchanged. Candidate Linux verification passed 402 shared-core tests and 1,465 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle, browser/Firefox (30 tests) and Python/catalog (79 tests) gates; the local Windows run retains the known 18 line-ending-sensitive boundary failures, physical-device ergonomics remain unverified, and production Home/private-price-book wiring remains unfinished.

The latest promoted code milestone is `d53b4fca70c3e84b32fab49c7b8aad789e4e92e3` (`Let Home save confirmed offline product identities`), verified by candidate workflow **33923409718** and milestone provenance workflow **33923881317**. Resolved Home rows now offer an explicit exact-identity choice from the signed offline catalog. The shopper must review and confirm a bounded candidate; only the existing source-revalidated exact-choice projector and asynchronous Saved transaction can persist a private product identity preference. Identity selection supplies no package quantity, price, currency, store, stock, availability or freshness, unknown rows remain query-only, and the fictional sample planner is unchanged and clearly disclosed. This reversible consumer-workspace slice adds no planner/projector duplication, Android networking, new persistence authority, ranking influence or provider economics. Candidate Linux verification passed 402 shared-core tests and 1,457 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle, browser/Firefox (30 tests) and Python/catalog (79 tests) gates; the local Windows run retains the known 18 line-ending-sensitive boundary failures, physical-device ergonomics remain unverified, and production Home/private-price-book wiring remains unfinished.

The promoted milestone is now `80b9efd5e33839e3f0dfd1b10e273c1a2547cfaf` (`Show private Home price observation date`), verified by candidate workflow **33919462842** and milestone provenance workflow **33920091991**. Home’s exact normalized name/package private-history context now says when the latest observation was recorded using a stable UTC date; a missing/zero timestamp remains `date not recorded`. Mixed package, currency, basis or promotion histories stay on the cautious name-only notice. The formatter is shared with the private-history review, and all copy keeps personal observations separate from current retailer pricing. This reversible read-only enhancement does not modify the planner/projector, exact money/quantity authority, evidence, current offers, store/availability, ranking, persistence, networking or provider economics. Candidate Linux verification passed 402 shared-core tests and 1,452 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle, browser/Firefox (30 tests) and Python/catalog (79 tests) gates; the Windows run retains the known 18 line-ending-sensitive boundary failures, physical-device ergonomics remain unverified, and production Home/private-price-book wiring remains unfinished.

The promoted milestone is now `8fc5629e99cca13ff2aca2b462ddaf5ee1bc1dbf` (`Show comparable personal price context on Home`), verified by candidate workflow **33917203335** and milestone provenance workflow **33917694467**. Home now forwards the fictional sample row’s normalized package quantity to its existing device-only private-history presentation. A matching normalized name/package with one currency, unit basis, price-selection and promotion shape can show the last recorded personal price and deterministic remembered unit-rate range; mixed package, currency, basis or promotion observations remain a cautious name-only notice. Copy says product identity, brand and store may differ and this is not live store pricing. This focused read-only enhancement does not modify the planner/projector, exact money/quantity authority, evidence, current offers, store/availability, ranking, persistence, networking or provider economics. Candidate Linux verification passed 402 shared-core tests and 1,451 Android tests with zero failures, lint/build/privacy/single-signer/release-bundle, browser/Firefox (30 tests) and Python/catalog (79 tests) gates; the Windows run retains the known 18 line-ending-sensitive boundary failures, physical-device ergonomics remain unverified, and production Home/private-price-book wiring remains unfinished.

The promoted milestone is now `f3afb2b274ad16dbc84454cc615277c2084ff124` (`Show exact item prices in Home plan`), verified by candidate workflow **33914080876** and milestone provenance workflow **33914604138**. Home’s existing plan rows now show `Included in plan: X CAD` for covered items whenever a candidate provides an exact breakdown. The shared candidate boundary validates exact key coverage, money specification and subtotal equality; planner/ranking decisions continue to use only their existing authoritative fields. Production and demo bridges supply the descriptive values, missing items remain unknown and legacy candidates remain compatible. Basket invalidates its local check-off scope when a projected included amount changes. This focused consumer-clarity slice adds no product, offer, availability, evidence, planner, ranking, networking, persistence or provider-economics authority. Candidate Linux Android tests/lint/build/privacy/single-signer/release, browser/Firefox and Python/catalog gates all passed; the Windows run retains the known 18 line-ending-sensitive boundary failures, physical-device ergonomics remain unverified, and production Home/private-price-book wiring remains unfinished.

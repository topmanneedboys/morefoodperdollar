...
`b4af05b6a38d233fb3e8ec85e0d95f491ccab0c8` — `Disable repeat submit during home refinement`

GitHub Actions candidate workflow **33704284840** completed successfully, and milestone provenance workflow **33704835479** completed successfully for the exact same SHA.

The latest Home refinement closes a semantic readiness gap after submission. When the immutable model is in `NEEDS_REFINEMENT` (an ambiguous choice or unresolved item), the renderer now disables the repeat Plan my shop action so the user is guided to the existing refinement/correction controls instead of an ineffective retry; editing the query or making the projected choice re-enables the normal action. This remains projector-owned presentation readiness with deterministic coverage: the controller and shared core still own resolution and planning, and no planner, ranking, pricing, evidence, persistence, clock, provider or network authority moved into the View. Clean-source verification passed 1,649 JVM tests (375 shared-core + 1,274 Android app), all 58 Android tasks, all 30 browser tests, Firefox lint with zero findings, APK privacy checks with no network permissions, one-signer verification and release-bundle provenance.

...
`46f45e55be1e0d1d40615136057144b5f6a6785c` — `Use accessibility-specific card child suppression`

GitHub Actions candidate workflow **33700115146** completed successfully, and milestone provenance workflow **33700715935** completed successfully for the exact same SHA.

The latest shared Home/Basket result-card refinement makes the accessibility contract match the renderer’s existing card-level summaries. Primary and optional-second-stop cards now hide only their decorative child labels through Android’s accessibility-tree flag, so TalkBack reaches one complete projected summary per card rather than repeating the same fields individually. The change is presentation-only and retains every projected field, visual line and exact typed result; no planner, ranking, pricing, evidence, persistence, clock, provider or network authority moved into the View. Clean-source verification passed 1,645 JVM tests (375 shared-core + 1,270 Android app), all 58 Android tasks, all 30 browser tests, Firefox lint with zero findings, APK privacy checks with no network permissions, one-signer verification and release-bundle provenance.

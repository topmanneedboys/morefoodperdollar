...
The promoted milestone is now `8f6f950c9326b1f6350d2415446e5821e6eada14` (`Explain Scan and Compare value advantage`), verified by candidate workflow **33907686165** and milestone provenance workflow **33908194019**. Scan & Compare’s existing READY projection now tells the shopper how much better a unique winner is than the next safe alternative, using a deterministic relative-gap percentage from the already-ranked exact unit rates; equal winners receive explicit tie guidance. This is presentation-only and uses BigDecimal/HALF_UP display rounding, so it does not alter ranking or add money, quantity, evidence, product, store, offer, availability, planner, networking or provider-economics authority. Existing manual exact-price, currency, package and like-for-like gates, private-memory rules and fictional Home/sample separation remain unchanged. Candidate Linux Android tests/lint/build/privacy/single-signer/release, browser/Firefox and Python/catalog gates all passed; the Windows run retains the known 18 line-ending-sensitive boundary failures, physical-device ergonomics remain unverified, and production Home/private-price-book wiring remains unfinished.

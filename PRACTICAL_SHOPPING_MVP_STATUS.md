...
`59d565e79c7fa6e3f5a0b28acb5fc7920bb1c5c0` — `Record deterministic offline catalog coverage`

GitHub Actions candidate workflow **33814661112**, offline-catalog workflow **33814661056**, and milestone provenance workflow **33815157624** completed successfully for the exact same SHA.

The weekly offline catalog coordinator now emits an atomic, deterministic `coverage-report.json` for each successful refresh. It reports selected identity-catalog coverage separately from current-offer coverage (`0` / `NOT_INCLUDED`) for each requested metro, repeats each verified manifest hash, and remains diagnostic-only; signed manifests and promotion pointers are still the authorities. Candidate current-offer metadata is checked explicitly before staging, and a rejected regression leaves the previous report and last-known-good pointers unchanged. This does not add prices, offers, availability, planner/ranking authority, Android networking, or a new consumer claim. Home’s preceding offline catalog result surface remains explicitly actionable without overstating what the catalog proves: a shopper can select a bounded display name, replace only the first whole unresolved list token, review the edited list, and press Plan My Shop themselves. Clean-source verification passed 400 shared-core tests and 1,337 Android app tests with zero failures, all 58 Android tasks, 61 Python tests, 30 browser tests, Firefox lint with zero findings, APK privacy inspection with no INTERNET/ACCESS_NETWORK_STATE, one-signer verification, and exact-SHA release-bundle provenance.

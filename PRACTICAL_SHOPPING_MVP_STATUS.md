...
The latest promoted code milestone is `4d205db3a3b144cc7725949c96f97e4de9d025aa` (`Add privacy-safe Compare Here result sharing`), verified by candidate workflow **33925898395** and milestone provenance workflow **33926352708**. Scan & Compare now has an explicit, review-first share action for a fully labelled READY comparison with at least two exact rows. The generic bounded card contains only the number of entered options and the existing exact best unit-rate (or an explicit tie), says it is based on entered values rather than live store pricing, and excludes product names, private history, receipts, location, account and source identifiers. Non-ready, blocked, estimated, conflicting and incomplete comparisons remain unshareable. This reversible consumer-clarity slice leaves the shared planner/projector, exact money/quantity authority, evidence, offers, stores, availability, persistence, Android networking, demo separation and provider-economics rules unchanged. Candidate Linux verification passed the full shared-core/Android test, lint/build/privacy/single-signer/release-bundle, browser/Firefox (30 tests) and Python/catalog (79 tests) gates; the local Windows run retains the known 18 line-ending-sensitive boundary failures, physical-device ergonomics remain unverified, and production Home/private-price-book wiring remains unfinished.

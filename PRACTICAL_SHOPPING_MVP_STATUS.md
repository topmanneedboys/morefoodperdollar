...
The promoted milestone is now `1d23138c8b0b3b5ab97baa6fba55df055daffe43` (`Add truthful Data Status surface`), verified by candidate workflow **33878830784** and milestone provenance workflow **33879410427**. The shell exposes a `Data status` action that states the actual bundled 30,000-record Canada-labelled identity rail, reports readable private-observation count or an unavailable fail-closed state, and keeps current offers (`0` authorized records), store directory, flyers and remote connectivity as separate “not included” boundaries. Home/Search examples are explicitly fictional sample data. This is presentation-only with deterministic helper, lifecycle and source-boundary tests; no planner, ranking, price, offer, store, stock, availability, provider-economics, persistence or network authority moved into the surface. Candidate Linux Android/browser/privacy/signing/release verification and Python/catalog tests passed. The Windows run retains the known 18 line-ending-sensitive boundary failures; physical-device barcode ergonomics and a real store directory remain open.

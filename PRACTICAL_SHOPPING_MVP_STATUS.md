...
The promoted milestone is `9b8b8b349586aeda0f71b5746be573d3221f12da` (`Remember confirmed Compare Here prices privately`), verified by candidate workflow **33843985987** and milestone provenance workflow **33844394607**. Compare Here now turns a successful, explicitly confirmed exact comparison into bounded private device memory using the typed facts already accepted by the exact route. Each snapshot keeps selected price basis, exact quantity/rate, optional exact promotion terms, observed time and a deterministic fingerprint; it carries no store/provider/availability/current-offer authority and is excluded from planner/ranking inputs. Atomic local storage rejects invalid or tampered documents, caps history at 256 entries and provides an explicit clear action. Only READY user actions persist; blocked, incomplete, incompatible, unconfirmed and restored-draft states remain non-persistent.

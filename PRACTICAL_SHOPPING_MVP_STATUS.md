...
`73acc24488beb8905fc93a2670066e0aedea082e` — `Announce shell destination changes accessibly`

GitHub Actions candidate workflow **33717058486** completed successfully, and milestone provenance workflow **33717481814** completed successfully for the exact same SHA.

The latest shell refinement closes an accessibility feedback gap. The existing top-level destination title is now a polite accessibility live region, so assistive technology can hear when Home, Search, Basket or Saved becomes selected without the shell interpreting navigation or shopping state. The main Saved surface still announces its projected unresolved display-metadata warning politely; existing Saved lifecycle status, Watch setup selection summary, fail-closed notice and foreground fact-resolution progress remain polite live regions; Watch policy setup still announces its projected validation notice and “Still needed” requirements card politely; Watch My Staples still announces projected readiness/economic status and exposes one coherent switch-candidate summary; Compare Here retains polite input/readiness and evaluated-result announcements with one-summary exact/blocked cards; Search result cards retain complete projected summaries with explicit fictional-sample context; and the shared projected Home/Basket result container remains a polite accessibility live region for newly planned recommendations. Clean-source verification passed 1,667 JVM tests (375 shared-core + 1,292 Android app), all 58 Android tasks, all 30 browser tests, Firefox lint with zero findings, one-signer verification and release-bundle provenance.

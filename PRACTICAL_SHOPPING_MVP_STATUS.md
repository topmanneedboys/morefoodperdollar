...
The promoted milestone is `ed9fc561b01dcdbb04c80bee965535aa8f81752d` (`Add first-class good price check`), verified by candidate workflow **33852574190** and milestone provenance **33853040970**. Home now exposes a dedicated, offline “Is this a good price?” action. The route accepts one bounded product entry and delegates exact parsing, package normalization, selected current/member price semantics and unit math to the existing Compare Here bridge. It reports only private history: no matching evidence, comparison with the last observation, or a personal range; it never claims a live market or store answer. Exact currency, package quantity, price basis and promotion facts remain strict, so estimated quantities, missing member prices, ambiguous currencies and malformed entries stay unknown/blocked. Successful checks create a source-labelled `CONFIRMED_GOOD_PRICE_CHECK` observation in the existing 256-entry atomic, integrity-checked device memory; the source is retained through codec round trips, and the memory remains outside planner/ranking/current-offer authority. Home and the new screen are owner-gated, display immutable projected content, and disclose that prices are not live. Only exact successful user actions persist; no adapter/core objects or raw user text cross into the renderer.
